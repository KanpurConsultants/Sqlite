...
Public Class ClsMain
    ...
    Public Const ModuleName As String = "Time Office"
...
#End Region
    ...
    Public Shared Function FGetCustomFieldFromV_Type(ByVal V_Type As String, ByVal Conn As SqlClient.SqlConnection) As String
    ...
    Private Sub FAddStructureFields(ByRef MdlTable() As AgLibrary.ClsMain.LITable)
        Try
            ...
            Dim I As Integer
            Dim J As Integer
            mQry = "Select CustomFields, HeaderTable, Ht.Name as HeaderTableName, LHT.Name as LogHeaderTableName  " & _
                   " From Voucher_Type " & _
                   " Left Join Sys.Objects HT On Voucher_Type.HeaderTable = HT.Object_ID " & _
                   " Left Join Sys.Objects LHT On Voucher_Type.LogHeaderTable = LHT.Object_ID " & _
                   " Where CustomFields Is Not Null "
            DtVType = AgL.FillData(mQry, AgL.GCn).Tables(0)


            For I = 0 To DtVType.Rows.Count - 1
                mQry = "Select HeaderField, HeaderFieldDataType, HeaderFieldLength From CustomFieldsDetail With (NoLock) Where Code = '" & DtVType.Rows(I)("CustomFields") & "' And Head Is Not Null  "
    ...
    'Function To Lock New Entry In Old Entry Point
    Public Shared Function FLockNewEntryInOldEntryPoint(ByVal JobProcess As String, ByVal V_Date As String, ByVal EntryMode As String) As Boolean
        Try
            Select Case JobProcess
                Case "THBCK"
                    If AgL.StrCmp(EntryMode, "Add") Then
                        MsgBox("Can't Add Entry.You Can Add Entry In New Entry Point.", MsgBoxStyle.Information)
                        FLockNewEntryInOldEntryPoint = True
                    Else
                        If CDate(V_Date) >= CDate("28/Aug/2013") Then
                            MsgBox("Can't Edit Entry.You Can Edit This Entry In New Entry Point.", MsgBoxStyle.Information)
                            FLockNewEntryInOldEntryPoint = True
                        End If
                    End If
            End Select
