...
#End Region
    ...
    Private Sub FrmSaleOrder_BaseFunction_IniGrid() Handles Me.BaseFunction_IniGrid
        ...
        With AgCL
            ...
            .AddAgTextColumn(Dgl1, Col1SaleOrder, 100, 0, Col1SaleOrder, IsApplicable_Order, True)
            ...
            .AddAgTextColumn(Dgl1, Col1SaleChallan, 100, 255, Col1SaleChallan, IsApplicable_Challan, True)
    ...
    Private Sub FrmSaleOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Save_InTrans
            ...
            If Dgl1.Item(Col1Item, I).Value <> "" Then
                If Dgl1.Item(ColSNo, I).Tag Is Nothing And Dgl1.Rows(I).Visible = True Then
                    ...
                    bSelectionQry += " Select " & AgL.Chk_Text(mSearchCode) & ", " & mSr & ", " &
                                        ...
                                        " " & AgL.Chk_Text(Dgl1.Item(Col1SaleOrderSr, I).Value) & ", " &
                                        ...
                                        " " & AgL.Chk_Text(Dgl1.Item(Col1SaleChallanSr, I).Value) & ", " &
                    ...
                    If Dgl1.Rows(I).Visible = True Then
                        If Dgl1.Rows(I).DefaultCellStyle.BackColor <> RowLockedColour Then
                            ...
                            mQry = " UPDATE SaleInvoiceDetail " &
                                    " SET " &
                                    " SaleOrder = " & AgL.Chk_Text(Dgl1.Item(Col1SaleOrder, I).Tag) & ", " &
                                    " SaleOrderSr = " & AgL.Chk_Text(Dgl1.Item(Col1SaleOrderSr, I).Value) & ", " &
                                    ...
                                    " SaleChallanSr = " & AgL.Chk_Text(Dgl1.Item(Col1SaleChallanSr, I).Value) & ", " &
        ...
        If bSelectionQry <> "" Then
            mQry = "Insert Into SaleInvoiceDetail(DocId, Sr, SaleOrder, SaleOrderSr, Item_Uid, Item, ItemInvoiced, Specification, SalesTaxGroupItem, " &
    ...
    Private Sub FPostInSaleChallan(ByVal Conn As SQLiteConnection, ByVal Cmd As SQLiteCommand)
        ...
        If AgL.XNull(AgL.Dman_Execute(mQry, AgL.GcnRead).ExecuteScalar) > 0 Then
            ...
            mQry = " INSERT INTO SaleChallanDetail(DocId, Sr, SaleOrder, Item, Specification, " &
                    ...
                    " Rate, Amount, Remark, Landed_Value,  " &
                    ...
                    " L.Rate, L.Amount, L.Remark, L.Landed_Value, " &
                    ...
                    " L.TotalDocDeliveryMeasure, L.TotalFreeDeliveryMeasure, L.TotalDeliveryMeasure,  " &
    ...
    Private Function FHPGD_PendingSaleChallan() As String
        ...
        mQry = " SELECT 'o' As Tick, VMain.SaleChallan As SaleChallan, " &
                ...
                " FROM ( " & FRetFillItemWiseSaleChallanQry("WHERE V_Date <= '" & TxtV_Date.Text & "' And SaleToParty = '" & TxtSaleToParty.Tag & "'", "") & " ) As VMain " &
    ...
    Private Function FRetFillItemWiseSaleChallanQry(ByVal HeaderConStr As String, ByVal LineConStr As String) As String
        FRetFillItemWiseSaleChallanQry = "SELECT Max(L.V_Nature) As V_Nature, Max(L.SaleOrder) As SaleOrder, Max(So.ReferenceNo) As SaleOrderNo, " &
                        ...
                        " Sum(L.DocQty) - IfNull(Max(Cd.DocQty), 0) as DocQty,   " &
                        " Sum(L.FreeQty) - IfNull(Max(Cd.FreeQty), 0) as FreeQty,   " &
    ...
    Private Function FHPGD_PendingSaleOrders() As String
        ...
        mQry = " SELECT 'o' As Tick, VMain.SaleOrder As SaleOrder, " &
                " Max(VMain.SaleOrderNo) AS SaleOrderNo, " &
                " Max(VMain.SaleOrderDate) as SaleOrderDate, " &
                " IfNull(Sum(VMain.Qty), 0) As [Qty]    " &
                " FROM ( " & FRetFillItemWiseSaleOrderQry("WHERE V_Date <= '" & TxtV_Date.Text & "' And SaleToParty = '" & TxtSaleToParty.Tag & "'", "") & " ) As VMain " &
                " GROUP BY VMain.SaleOrder " &
    ...
    Private Function FRetFillItemWiseSaleOrderQry(ByVal HeaderConStr As String, ByVal LineConStr As String) As String
        FRetFillItemWiseSaleOrderQry = "SELECT '" & RbtForSaleOrder.Text & "' As V_Nature, L.SaleOrder, " &
                        " Max(H.ReferenceNo) As SaleOrderNo, L.SaleOrderSr , Max(H.V_Date) As SaleOrderDate, " &
                        " Max(L.RatePerQty) as SaleOrderRatePerQty, Max(L.RatePerMeasure) As SaleOrderRatePerMeasure, " &
                        " Max(L.Item) As Item, Max(I.ManualCode) as ItemManualCode,  Max(I.Description) as Item_Name, " &
                        " Max(L.Specification) as Specification," &
                        " Max(L.SalesTaxGroupItem) SalesTaxGroupItem, " &
                        " Max(L.BillingType) As BillingType, Max(L.RateType) As RateType, " &
                        ...
                        " FROM (  " &
                        "    SELECT DocID, V_Type, ReferenceNo, V_Date   " &
                        "    FROM SaleOrder  " & HeaderConStr & " " &
                        " ) AS  H   " &
                        " LEFT JOIN SaleOrderDetail L  ON H.DocID = L.SaleOrder " &
                        " Left Join Item I  On L.Item  = I.Code   " &
                        " LEFT JOIN Voucher_Type Vt  ON H.V_Type = Vt.V_Type    " &
                        " Left Join (   " &
                        ...
                        " ) AS CD ON L.DocId = CD.SaleOrder AND L.Sr = CD.SaleOrderSr " &
                        " LEFT JOIN Unit U On L.Unit = U.Code   " &
                        " LEFT JOIN Unit U1 On L.MeasureUnit = U1.Code   " &
                        " GROUP BY L.SaleOrder, L.SaleOrderSr " &
                        " Having Sum(L.Qty) - IfNull(Max(Cd.Qty), 0) > 0  "
    ...
    Private Function FRetFillItemWiseSaleInvoiceQry(ByVal HeaderConStr As String, ByVal LineConStr As String) As String
        FRetFillItemWiseSaleInvoiceQry = "SELECT '" & RbtSaleReturn.Text & "' As V_Nature, " &
                        ...
                        " Max(L.MRP) As MRP, Max(L.ExpiryDate) As ExpiryDate, " &
                        " Max(L.ReferenceDocId) As ReferenceDocId, Max(L.ReferenceDocIdSr) As ReferenceDocIdSr, " &
                        " Max(L.SaleChallan) As SaleChallan, Max(L.SaleChallanSr) As SaleChallanSr, " &
    ...
    Private Sub FCreateHelpItem()
        ...
        If RbtSaleReturn.Checked Then
            mQry = " SELECT Max(L.Item) As Code, " &
                        ...
                        " Max(L.SaleChallan) As SaleChallan, Max(L.SaleChallanSr) As SaleChallanSr, Max(Sc.ReferenceNo) As SaleChallanNo, " &
                        " Max(L.SaleOrder) As SaleOrder, Max(L.SaleOrderSr) As SaleOrderSr, Max(So.ReferenceNo) As SaleOrderRefNo " &
                        " FROM (    " &
                        "     SELECT DocID, V_Type, ReferenceNo, V_Date " &
                        "     FROM SaleInvoice      " &
                        ...
                        " LEFT JOIN SaleChallan So ON L.SaleOrder = So.DocId " &
        ...
        ElseIf RbtForStock.Checked Then
            mQry = " SELECT Max(I.Code) AS Code, Max(I.Description) AS Description, " &
                     ...
                     " Max(U.DecimalPlaces) As QtyDecimalPlaces, Max(U1.DecimalPlaces) As MeasureDecimalPlaces, " &
                     " Max(I.BillingOn) as BillingType, Null As SaleChallan, Null As SaleChallanNo, Null As SaleChallanSr, " &
        ...
        ElseIf RbtForSaleOrder.Checked Then
            mQry = " SELECT Max(L.Item) As Code, Max(I.Description) as Description, " &
                    ...
                    " Max(I.Unit) as Unit,   Max(H.V_Type) || '-' ||  Max(H.ReferenceNo) AS SaleOrderRefNo, " &
                    " Sum(L.TotalMeasure) - IfNull(Sum(Cd.TotalMeasure), 0) as [Bal.Measure],   " &
                    " Max(I.MeasureUnit) MeasureUnit, Max(L.MeasurePerPcs) as MeasurePerPcs, " &
                    " Max(U.DecimalPlaces) as QtyDecimalPlaces,  " &
                    " Max(U1.DecimalPlaces) as MeasureDecimalPlaces,   " &
                    " Max(I.SalesTaxPostingGroup) SalesTaxPostingGroup, " &
                    " Max(L.BillingType) As BillingType,  " &
                    " Null As SaleChallan, Null As SaleChallanSr, Null As SaleChallanNo, " &
                    " Null As StockInDocId, Null As StockInDocIdSr, Null As StockInNo, " &
                    " Null As ExpiryDate, Null As MRP, Null As LotNo, " &
                    " Max(L.Rate) as Sale_Rate,   " &
                    " Null As PurchaseRate, " &
                    " L.SaleOrder, L.SaleOrderSr, " &
                    " Max(L.RatePerQty) as RatePerQty, Max(L.RatePerMeasure) as RatePerMeasure, Null As SaleInvoice, Null As SaleInvoiceSr, Null As SaleInvoiceNo  " &
                    " FROM (  " &
                    "     SELECT DocID, V_Type, ReferenceNo, V_Date , PartyOrderNo  " &
                    "     FROM SaleOrder    " &
                    "     WHERE SaleToParty ='" & TxtSaleToParty.Tag & "'   " &
                    "     And Div_Code = '" & TxtDivision.Tag & "'   " &
                    "     AND Site_Code = '" & TxtSite_Code.Tag & "'   " &
        ...
        ElseIf RbtForSaleChallan.Checked Then
            mQry = " SELECT Max(L.Item) As Code, Max(I.Description) as Description, " &
                    ...
                    " Max(I.SalesTaxPostingGroup) SalesTaxPostingGroup, " &
                    " Max(L.BillingType) As BillingType,  " &
                    " L.SaleChallan As SaleChallan, L.SaleChallanSr As SaleChallanSr, Max(H.V_Type) || '-' ||  Max(H.ReferenceNo) AS SaleChallanNo, " &
                    " Null As StockInDocId, Null As StockInDocIdSr, Null As StockInNo, " &
                    " Null As ExpiryDate, Null As MRP, Null As LotNo, " &
                    " Max(L.Rate) as Sale_Rate,   " &
                    " Null As PurchaseRate, " &
                    " Max(L.SaleOrder) As SaleOrder, Max(H.V_Type) || '-' ||  Max(So.ReferenceNo) AS SaleOrderRefNo, Max(L.SaleOrderSr) As SaleOrderSr, " &
                    " Max(L.RatePerQty) as RatePerQty, Max(L.RatePerMeasure) as RatePerMeasure, Null As SaleInvoice, Null As SaleInvoiceSr, Null As SaleInvoiceNo  " &
                    " FROM (  " &
                    "     SELECT DocID, V_Type, ReferenceNo, V_Date " &
                    "     FROM SaleChallan    " &
                    ...
                    " LEFT JOIN Voucher_Type Vt  ON H.V_Type = Vt.V_Type    " &
                    " Left Join (   " &
                    "     SELECT L.SaleChallan, L.SaleChallanSr, sum (L.Qty) AS Qty, Sum(L.TotalMeasure) as TotalMeasure      " &
                    " 	  FROM SaleInvoiceDetail L     " &
                    " 	  GROUP BY L.SaleChallan, L.SaleChallanSr   " &
                    " 	) AS CD ON L.DocId = CD.SaleChallan AND L.Sr = CD.SaleChallanSr   " &
                    " LEFT JOIN Unit U On L.Unit = U.Code   " &
