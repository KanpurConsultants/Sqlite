...
#End Region
    ...
    Private Sub FrmPurchRequisition_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = " SELECT H.DocID, H.V_Date AS [Requisition Date], H.ReferenceNo AS [Requisition No.] , SG.DispName AS [Requisition By], D.Description AS Department, H.Remarks, " & _
                            " SM.Name AS Site_Name, DIV.Div_Name  " & _
                            " FROM Requisition H " & _
                            " LEFT JOIN SubGroup SG ON SG.SubCode = H.RequisitionBy  " & _
                            " LEFT JOIN Department D ON D.Code = H.Department  " & _
                            " LEFT JOIN SiteMast SM ON SM.Code = H.Site_Code " & _
                            " LEFT JOIN Division DIV ON DIV.Div_Code = H.Div_Code  " & _
                            " Left Join Voucher_Type Vt On H.V_Type = Vt.V_Type  " & _
    ...
    Private Sub FrmPurchRequisition_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As System.Data.SqlClient.SqlConnection, ByVal Cmd As System.Data.SqlClient.SqlCommand) Handles Me.BaseEvent_Save_InTrans
        Dim I As Integer, mSr As Integer
        Dim bSelectionQry As String = ""
        mQry = " UPDATE Requisition " & _
                " SET " & _
                " ReferenceNo = " & AgL.Chk_Text(TxtManualRefNo.Text) & ", " & _
                " Department = " & AgL.Chk_Text(TxtDepartment.Tag) & ", " & _
                " RequisitionBy = " & AgL.Chk_Text(TxtRequisitionBy.Tag) & ", " & _
        ...
        mSr = AgL.VNull(AgL.Dman_Execute("Select Max(Sr) From RequisitionDetail With (NoLock) Where DocID = '" & mSearchCode & "'", AgL.GcnRead).ExecuteScalar)
        For I = 0 To Dgl1.RowCount - 1
            If Dgl1.Item(Col1Item, I).Value <> "" Then
                If Dgl1.Item(ColSNo, I).Tag Is Nothing And Dgl1.Rows(I).Visible = True Then
                    ...
                    bSelectionQry += " Select " & AgL.Chk_Text(SearchCode) & ", " & mSr & ", " & _
                            ...
                            " " & AgL.Chk_Text(Dgl1.Item(Col1RequireDate, I).Value) & ", " & _
                ...
                Else
                    If Dgl1.Rows(I).Visible = True Then
                        If Dgl1.Rows(I).DefaultCellStyle.BackColor <> RowLockedColour Then
                            mQry = " UPDATE RequisitionDetail " & _
                                    ...
                                    " RequireDate = " & AgL.Chk_Text(Dgl1.Item(Col1RequireDate, I).Value) & ", " & _
        ...
        mQry = " INSERT INTO RequisitionDetail (DocId, Sr,	Item, Qty, " & _
                " Unit,	MeasurePerPcs,	MeasureUnit,	TotalMeasure, " & _
    ...
    Private Sub FrmPurchRequisition_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        Dim strQryPurchIndent As String = " SELECT L.Requisition, L.RequisitionSr,  sum(L.Qty) AS IndQty " & _
                    " FROM PurchIndentReq L  " & _
                    " WHERE isnull(L.Requisition,'') <> ''  " & _
                    " GROUP BY L.Requisition, L.RequisitionSr "

        mQry = " SELECT H.DocID, H.V_Type, H.ReferenceNo, H.V_Prefix, H.V_Date, H.V_No, H.RequisitionBy, H.Department, H.Remarks, " & _
                " SG.DispName AS ReqByName, D.Description AS DepartmentName, L.Sr, L.Item, L.Qty, L.Unit, L.MeasurePerPcs, L.ApproveBy, L.ApproveQty , L.Specification, " & _
                " L.MeasureUnit, L.TotalMeasure, L.RequireDate, L.Remark AS LineRemark, I.Description AS ItemName, I.ManualCode AS ItemCode,   " & _
                " U.DecimalPlaces as QtyDecimalPlaces, MU.DecimalPlaces as MeasureDecimalPlaces, " & _
                " ( Case When IsNull(PI.IndQty,0) > 0 Then 1 ELSE CASE WHEN IsNull(PI.IndQty,0) > 0 THEN 1 ELSE 0 END END ) as RowLocked " & _
                " FROM " & _
                " ( " & _
                "  SELECT * FROM Requisition WHERE DocID = '" & SearchCode & "' " & _
                " ) H  " & _
                " LEFT JOIN SubGroup SG ON SG.SubCode = H.RequisitionBy  " & _
                " LEFT JOIN Department D ON D.Code = H.Department  " & _
                ...
                " Left Join ( " & strQryPurchIndent & " ) As PI On L.DocID = PI.Requisition and L.Sr = PI.Requisitionsr " & _
                " Order By Sr "
    ...
    Private Sub FCreateHelpItem(ByVal ColumnName As String)
        ...
        Select Case ColumnName
            Case Col1Item
                mQry = " SELECT I.Code, I.Description AS ItemDesc, I.ManualCode AS ItemCode, I.Unit, " & _
                        " I.Measure As MeasurePerPcs, I.MeasureUnit, U.DecimalPlaces As QtyDecimalPlaces, U1.DecimalPlaces As MeasureDecimalPlaces " & _
                        " FROM Item I " & _
            ...
            Case Col1ItemCode
                mQry = " SELECT I.Code, I.ManualCode AS ItemCode, I.Description AS ItemDesc, I.Unit, " & _
                        " I.Measure As MeasurePerPcs, I.MeasureUnit, U.DecimalPlaces As QtyDecimalPlaces, U1.DecimalPlaces As MeasureDecimalPlaces " & _
                        " FROM Item I " & _
                        " LEFT JOIN Unit U On I.Unit = U.Code " & _
                        " LEFT JOIN Unit U1 On I.MeasureUnit = U1.Code " & _
    ...
    Private Sub TempPurchIndent_BaseEvent_Topctrl_tbDel(ByRef Passed As Boolean) Handles Me.BaseEvent_Topctrl_tbDel
        ...
        mQry = " SELECT isnull(sum(L.ApproveQty),0) AS ApproveQty " & _
    ...
    Private Sub FCreateHelpSubgroup(ByVal sender As AgControls.AgTextBox)
        ...
        mQry = " SELECT H.SubCode AS Code, H.DispName AS [Employee Name], H.ManualCode AS [Employee Code], " & _
           ...
           " Where IsNull(H.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') =  '" & AgTemplate.ClsMain.EntryStatus.Active & "' AND H.MasterType = '" & ClsMain.MasterType.Employee & "'  " & strCond
