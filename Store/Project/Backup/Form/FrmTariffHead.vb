...
#End Region
    ...
    Private Sub FrmYarn_BaseEvent_Data_Validation(ByRef passed As Boolean) Handles Me.BaseEvent_Data_Validation
        ...
        If Topctrl1.Mode = "Add" Then
            mQry = "Select count(*) From TariffHead Where ManualCode ='" & TxtManualCode.Text & "' "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Code Already Exist!")

            mQry = "Select count(*) From TariffHead Where Description='" & TxtDescription.Text & "' "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Description Already Exist!")
        Else
            mQry = "Select count(*) From TariffHead Where ManualCode ='" & TxtManualCode.Text & "' And Code < >'" & mInternalCode & "' "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Code Already Exist!")

            mQry = "Select count(*) From TariffHead Where Description='" & TxtDescription.Text & "' And Code <> '" & mInternalCode & "' "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Description Already Exist!")
        End If
    End Sub
    ...
    Public Overridable Sub FrmYarn_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = "SELECT I.Code As SearchCode, I.ManualCode As Code, I.Description  FROM TariffHead I "
        AgL.PubFindQryOrdBy = "[Description]"
    End Sub

    Private Sub FrmYarn_BaseEvent_Form_PreLoad() Handles Me.BaseEvent_Form_PreLoad
        MainTableName = "TariffHead"
        LogTableName = "TariffHead_Log"
    End Sub
    ...
    Private Function FGetRelationalData() As Boolean
        Try
            mQry = " Select Count(*) From Item Where TariffHead = '" & mSearchCode & "'"
            If AgL.VNull(AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar) > 0 Then
                MsgBox(" Data Exists For Item " & TxtDescription.Text & " In Item Master . Can't Delete Entry", MsgBoxStyle.Information)
