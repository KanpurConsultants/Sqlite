Public Class FrmPartyRateGroup
...
#End Region
    ...
    Private Sub FrmYarn_BaseEvent_Data_Validation(ByRef passed As Boolean) Handles Me.BaseEvent_Data_Validation
        ...
        If Topctrl1.Mode = "Add" Then
            mQry = "Select count(*) From SubGroup Where Name ='" & TxtDescription.Text & "' And " & AgTemplate.ClsMain.RetDivFilterStr & "  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Description Already Exist!")
        Else
            mQry = "Select count(*) From SubGroup Where Name ='" & TxtDescription.Text & "' And SubCode <> '" & mSearchCode & "' And " & AgTemplate.ClsMain.RetDivFilterStr & "  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Description Already Exist!")
        End If
    End Sub

    Public Overridable Sub FrmYarn_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        Dim mConStr$ = " Where I.MasterType In ('" & AgTemplate.ClsMain.SubgroupType.PartyRateGroup & "')  "

        AgL.PubFindQry = "SELECT I.SubCode, I.Name as SubGroup_Rate_Group  " & _
                        " FROM SubGroup I  " & mConStr
    ...
    Private Sub FrmYarn_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As System.Data.SqlClient.SqlConnection, ByVal Cmd As System.Data.SqlClient.SqlCommand) Handles Me.BaseEvent_Save_InTrans
        mQry = "UPDATE SubGroup " & _
                " SET " & _
                " Name = " & AgL.Chk_Text(TxtDescription.Text) & ", " & _
                " DispName = " & AgL.Chk_Text(TxtDescription.Text) & ", " & _
                " MasterType = " & AgL.Chk_Text(AgTemplate.ClsMain.SubgroupType.PartyRateGroup) & " " & _
                " Where SubCode = '" & SearchCode & "' "
    ...
    Private Sub FrmQuality1_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        With DsTemp.Tables(0)
            If .Rows.Count > 0 Then
                ...
                TxtDescription.Text = AgL.XNull(.Rows(0)("Name"))
            End If
        End With
    End Sub
    ...
    Private Sub FrmYarn_BaseFunction_FIniMast(ByVal BytDel As Byte, ByVal BytRefresh As Byte) Handles Me.BaseFunction_FIniMast
        Dim mConStr$ = ""
        mConStr = "WHERE 1=1   " & AgL.RetDivisionCondition(AgL, "Div_Code") & " And I.MasterType In ('" & AgTemplate.ClsMain.SubgroupType.PartyRateGroup & "')  "
        mQry = "Select I.SubCode As SearchCode " & _
                " From SubGroup I " & mConStr & _
                " And IsNull(I.IsDeleted,0)=0 Order By I.Name "
    ...
    Private Sub FrmSubGroup_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        AgL.WinSetting(Me, 300, 885)
    End Sub

    Private Sub TxtSubGroupCategory_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs)
                ...
                Case TxtDescription.Name
                    If e.KeyCode <> Keys.Enter Then
                        If TxtDescription.AgHelpDataSet Is Nothing Then
                            mQry = "Select SubCode, Name As Name " & _
                                  " From SubGroup " & _
                                  " Where MasterType = '" & AgTemplate.ClsMain.SubgroupType.PartyRateGroup & "'" & _
                                  " Order By Name"
