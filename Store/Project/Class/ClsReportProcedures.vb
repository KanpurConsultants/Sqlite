    ...
    Private Sub ProcRequisitionStatusReport()
        ...
        Try
            ...
            If ReportFrm.FGetText(4) = "Pending To Issue" Then
                mCondStr = mCondStr & " AND L.ApproveQty - IfNull(VIS.RecQty,0) > 0 "
            End If
            ...
            mQry = " SELECT H.DocID, H.V_Date, H.ReferenceNo, H.Remarks, D.Description AS DepartmentName, " & _
                    ...
                    " E.Caption_Dimension1, E.Caption_Dimension2, D1.Description AS D1Desc,D2.Description AS D2Desc," & _
                    " VI.RecNo, VI.RecDate, IfNull(VI.RecQty,0) AS RecQty, L.ApproveQty - IfNull(VIS.RecQty,0) AS TotalBalQty, VPI.IndentNo, VPI.IndentDate, VPI.IndQty " & _
                    " FROM Requisition H " & _
                    " LEFT JOIN RequisitionDetail L ON L.DocId = H.DocID  " & _
                    " LEFT JOIN Department D ON D.Code = H.Department  " & _
                    " LEFT JOIN SubGroup SG ON SG.SubCode = H.RequisitionBy  " & _
...
#Region "Item Issue Report"
    Private Sub ProcItemIssueReport()
        ...
        Try
            ...
            mQry = " SELECT H.DocID, H.V_Type, H.V_Date, H.ManualRefNo, Sg.DispName + (Case When IfNull(Sg.CityCode,'')<>'' Then ', ' + C.CityName Else '' End) as PartyName, H.Remarks + L.Remarks AS Remarks, " & _
                    " E.Caption_Dimension1, E.Caption_Dimension2, D1.Description AS D1Desc,D2.Description AS D2Desc," & _
                    ...
                    " FROM ( SELECT SH.*, Vt.Description as VoucherTypeDesc FROM StockHead SH  LEFT JOIN Voucher_Type Vt  On Vt.V_Type = SH.V_Type WHERE Vt.NCat = '" & AgTemplate.ClsMain.Temp_NCat.StoreIssue & "' ) H " & _
    ...
    Private Sub ProcMaterialIssueSummary()
        ...
        Try
            ...
            mQry = " SELECT H.V_Date, H.VoucherTypeDesc, G.Description AS GodownDesc,  " & _
                    ...
                    " FROM StockHead SH  " & _
                    " LEFT JOIN Voucher_Type Vt  On Vt.V_Type = SH.V_Type " & _
                    " WHERE Vt.NCat = '" & AgTemplate.ClsMain.Temp_NCat.StoreIssue & "' ) H " & _
                    " LEFT JOIN StockHeadDetail L  ON L.DocID = H.DocID  " & _
                    " Left Join Subgroup Sg  on H.SubCode = Sg.SubCode " & _
                    " Left Join City C  on Sg.CityCode = C.CityCode  " & _
                    " LEFT JOIN Godown G  ON G.Code = H.FromGodown  " & _
                    " Left Join Process P  on H.Process = P.NCat " & _
                    " LEFT JOIN Item I  ON I.Code = L.Item  " & _
    ...
    Private Sub ProcItemReceiveReport()
        ...
        Try
            ...
            mQry = " SELECT H.DocID, H.V_Type, H.V_Date, H.ManualRefNo, Sg.DispName + (Case When IfNull(Sg.CityCode,'')<>'' Then ', ' + C.CityName Else '' End) as PartyName, H.Remarks + L.Remarks AS Remarks, " & _
                    " E.Caption_Dimension1, E.Caption_Dimension2, D1.Description AS D1Desc,D2.Description AS D2Desc," & _
                    " P.Description as ProcessDesc, H.VoucherTypeDesc, I.Description AS ItemDesc, L.Unit, L.LotNo, U.DecimalPlaces, UM.DecimalPlaces AS MeasureDecimalPlace, L.Qty, L.TotalMeasure, L.MeasureUnit, H.Remarks AS HeaderRemark, L.Remarks AS LineRemark  " & _
                    " FROM ( SELECT SH.*, Vt.Description as VoucherTypeDesc FROM StockHead SH  LEFT JOIN Voucher_Type Vt  On Vt.V_Type = SH.V_Type WHERE Vt.NCat = '" & AgTemplate.ClsMain.Temp_NCat.StoreReceive & "' ) H " & _
                    " LEFT JOIN StockHeadDetail L  ON L.DocID = H.DocID  " & _
                    " Left Join Subgroup Sg  on H.SubCode = Sg.SubCode " & _
                    " Left Join City C  on Sg.CityCode = C.CityCode  " & _
                    " Left Join Process P  on H.Process = P.NCat " & _
    ...
    Private Sub ProcStockTransferSummary()
        ...
        Try
            ...
            mQry = " SELECT H.V_Date, H.VoucherTypeDesc, FG.Description AS FromGodownDesc, TG.Description AS ToGodownDesc,  " & _
                    ...
                    " Left Join Subgroup Sg  on H.SubCode = Sg.SubCode " & _
                    " Left Join City C  on Sg.CityCode = C.CityCode  " & _
                    " LEFT JOIN Godown FG  ON FG.Code = H.FromGodown  " & _
                    " LEFT JOIN Godown TG  ON TG.Code = H.ToGodown " & _
                    " Left Join Process P  on H.Process = P.NCat " & _
                    " LEFT JOIN Item I  ON I.Code = L.Item  " & _
                    " LEFT JOIN ItemGroup IG ON I.ItemGroup = IG.Code  " & _
                    " LEFT JOIN ItemCategory IC ON IG.ItemCategory = IC.Code " & _
...
#Region "Stock Transfer Report"
    Private Sub ProcStockTransferReport()
        ...
        Try
            ...
            mQry = " SELECT H.DocID, H.V_Type, H.V_Date, H.ManualRefNo, Sg.DispName + (Case When IfNull(Sg.CityCode,'')<>'' Then ', ' + C.CityName Else '' End) as PartyName, IfNull(H.Remarks,'') + IfNull(L.Remarks,'') AS Remarks,  GF.Description AS FromGoodownDesc ,GT.Description AS ToGoodownDesc, " & _
                    " E.Caption_Dimension1, E.Caption_Dimension2, D1.Description AS D1Desc,D2.Description AS D2Desc," & _
                    ...
                    " FROM ( SELECT SH.*, Vt.Description as VoucherTypeDesc FROM StockHead SH  LEFT JOIN Voucher_Type Vt  On Vt.V_Type = SH.V_Type WHERE Vt.NCat IN ('" & AgTemplate.ClsMain.Temp_NCat.StockTransfer & "','YITP' ) ) H " & _
                    " LEFT JOIN StockHeadDetail L  ON L.DocID = H.DocID  " & _
                    " Left Join Subgroup Sg  on H.SubCode = Sg.SubCode " & _
                    " Left Join City C  on Sg.CityCode = C.CityCode  " & _
                    " Left Join Process P  on H.Process = P.NCat " & _
    ...
    Private Sub ProcPhysicalStockReport()
        ...
        Try
            ...
            mQry = " SELECT H.DocID, H.V_Type, H.V_Date, H.ManualRefNo, Sg.DispName + (Case When IfNull(Sg.CityCode,'')<>'' Then ', ' + C.CityName Else '' End) as PartyName, H.Remarks + L.Remarks AS Remarks, " & _
                    ...
                    " " & strGrpFld & " as GrpField, " & strGrpFldDesc & " as GrpFieldDesc, " & strGrpFldHead & " as GrpFieldHead " & _
                    " FROM ( SELECT SH.*, Vt.Description as VoucherTypeDesc FROM StockHead SH  LEFT JOIN Voucher_Type Vt  On Vt.V_Type = SH.V_Type WHERE Vt.NCat = '" & AgTemplate.ClsMain.Temp_NCat.PhysicalStockEntry & "' ) H " & _
                    " LEFT JOIN StockHeadDetail L  ON L.DocID = H.DocID  " & _
                    " Left Join Subgroup Sg  on H.SubCode = Sg.SubCode " & _
                    " Left Join City C  on Sg.CityCode = C.CityCode  " & _
    ...
    Private Sub ProcStockInHand()
        ...
        Try
            ...
            If AgL.StrCmp(ReportFrm.FGetText(3), "Measure") Then
                ...
                OpeningUnit1 = "Sum(IfNull(S.Measure_Rec,0)) - Sum(IfNull(S.Measure_Iss,0))"
                ReceiveUnit1 = "IfNull(S.Measure_Rec,0)"
                IssueUnit1 = "IfNull(S.Measure_Iss,0)"
            Else
                Unit1Head = "'Qty'"
                Unit1 = "I.Unit"
                Unit1DecimalPlace = "U.DecimalPlaces"

                OpeningUnit1 = "Sum(IfNull(S.Qty_Rec,0)) - Sum(IfNull(S.Qty_Iss,0))"
                ReceiveUnit1 = "IfNull(S.Qty_Rec,0)"
                IssueUnit1 = "IfNull(S.Qty_Iss,0)"
            End If
            ...
            ElseIf ReportFrm.FGetText(4) = "Godown and Process" Then
                mGroupOnValue = "IfNull(P.StockHead,P.Description)"
            ...
            If IsExcludeDimension1 = 1 Then mStockCondStr = mStockCondStr & " AND IfNull(S.Dimension1,'') ='' "
            If IsExcludeDimension2 = 1 Then mStockCondStr = mStockCondStr & " AND IfNull(S.Dimension2,'') ='' "
            If IsExcludeProcess = 1 Then mStockCondStr = mStockCondStr & " AND IfNull(S.Process,'') ='' "
            ...
            mLotNoFieldName = "Case When " & IsGroupOnLotNo & " = 1 And IfNull(Isd.IsRequired_LotNo,0) <> 0 Then S.LotNo Else Null End "
            ...
            Else
                ...
                ElseIf AgL.StrCmp(ReportFrm.FGetText(6), "Period Negative") Then
                    mBalcondStr = " AND Round(clg.closingQty,4) < 0 " & _
                                  " AND Round(IfNull(clg.closingQty,0),4) < Round(IfNull(OP.OpeningQty,0),4) "
                ...
                If AgL.StrCmp(ReportFrm.FGetText(6), "Period Negative") Then
                    mItemQry = " SELECT clg.Item, " & _
                                ...
                                " IfNull(Sum(S.Qty_Rec),0)-IfNull(Sum(S.Qty_Iss),0) AS closingQty " & _
                                " FROM Stock S  " & _
                                ...
                                " (CASE WHEN " & IsGroupOnDimension1 & " = 1 THEN S.Dimension1 ELSE NULL End) AS Dimension1, " & _
                                " (CASE WHEN " & IsGroupOnDimension2 & " = 1 THEN S.Dimension2 ELSE NULL End) AS Dimension2, " & _
                                " IfNull(Sum(S.Qty_Rec),0)-IfNull(Sum(S.Qty_Iss),0) AS OpeningQty " & _
                                " FROM Stock S  " & _
                                " LEFT JOIN Item I  ON S.Item = I.Code " & _
                                ...
                                " AND IfNull(clg.Godown,'') = IfNull(OP.Godown,'') " & _
                                " AND IfNull(clg.process,'') = IfNull(OP.process,'') " & _
                                " AND IfNull(clg.LotNo,'') = IfNull(OP.LotNo,'') " & _
                                " AND IfNull(clg.Dimension1,'') = IfNull(OP.Dimension1,'') " & _
                                " AND IfNull(clg.Dimension2,'') = IfNull(OP.Dimension2,'') " & _
                ...
                mParentQry = " [" & TUID & "] As VItem " & _
                            ...
                            " AND " & IIf(IsGroupOnGodown = 1, "IfNull(S.Godown,'')", "''") & " = IfNull(VItem.Godown,'') " & _
                            " AND " & IIf(IsGroupOnProcess = 1, "IfNull(S.Process,'')", "''") & " = IfNull(VItem.Process,'') " & _
                            " AND " & IIf(IsGroupOnLotNo = 1, "IfNull(S.LotNo,'')", "''") & " = IfNull(VItem.LotNo,'') " & _
                            " AND " & IIf(IsGroupOnDimension1 = 1, "IfNull(S.Dimension1,'')", "''") & " = IfNull(VItem.Dimension1,'') " & _
                            " AND " & IIf(IsGroupOnDimension2 = 1, "IfNull(S.Dimension2,'')", "''") & " = IfNull(VItem.Dimension2,'') "
            End If
            ...
            StrOpening = " SELECT S.Site_Code, '' AS SubCode, S.Godown, " & mGroupOn1Value & " AS Process, " & _
                        ...
                        " " & mGroupOn1 & " " & _
                        " Having Sum(IfNull(S.Qty_Rec,0)) - Sum(IfNull(S.Qty_Iss,0)) <> 0 "



            StrReceive = " SELECT S.Site_Code, S.SubCode, S.Godown, S.Process, " & _
                    " S.V_Date, S.DocID, S.V_No, S.RecID, S.V_Type, Vt.Description As TransactionType, IfNull(Sg.Name,'From Godown - ' + G.Description) AS Name, " & _
                    ...
                    " LEFT JOIN SubGroup Sg On S.SubCode = Sg.SubCode " & _
                    " LEFT JOIN StockHead SH  ON SH.DocID = S.DocID " & _
                    " LEFT JOIN Godown G  ON G.Code = SH.FromGodown " & _
                    " LEFT JOIN ItemSiteDetail Isd On S.Item = Isd.Code And Isd.Div_Code = S.Div_Code And Isd.Site_Code = S.Site_Code " & _
                    " LEFT JOIN Voucher_Type Vt On S.V_Type = Vt.V_Type " & _
                    " WHERE S.V_Date BETWEEN " & AgL.Chk_Text(ReportFrm.FGetText(0)) & " And " & AgL.Chk_Text(ReportFrm.FGetText(1)) & " AND IfNull(S.Qty_Rec,0) <> 0 " & _
                    " " & mStockCondStr & " "



            StrIssue = " SELECT S.Site_Code, S.SubCode, S.Godown, S.Process, " & _
                    " S.V_Date, S.DocID , S.V_No, S.RecID, S.V_Type , Vt.Description As TransactionType, IfNull(Sg.Name,'To Godown - ' + G.Description) AS Name, " & _
                    " S.Item , S.Dimension1, S.Dimension2, I.Unit AS Unit, " & _
                    " " & mLotNoFieldName & " As LotNo, " & _
                    ...
                    " LEFT JOIN SubGroup  Sg On S.SubCode = Sg.SubCode " & _
                    " LEFT JOIN StockHead SH  ON SH.DocID = S.DocID " & _
                    " LEFT JOIN Godown G  ON G.Code = SH.ToGodown " & _
                    " LEFT JOIN ItemSiteDetail Isd On S.Item = Isd.Code And Isd.Div_Code = S.Div_Code And Isd.Site_Code = S.Site_Code " & _
                    " LEFT JOIN Voucher_Type Vt On S.V_Type = Vt.V_Type " & _
                    " WHERE S.V_Date BETWEEN " & AgL.Chk_Text(ReportFrm.FGetText(0)) & " And " & AgL.Chk_Text(ReportFrm.FGetText(1)) & " AND IfNull(S.Qty_Iss,0) <> 0  " & _
                    " " & mStockCondStr & " "
...
#Region "Stock In Process"
    Private Sub ProcStockInProcess()
        ...
        Try
            ...
            'mLotNoFieldName = " Case When '" & ReportFrm.FGetText(12) & "' = 'Yes' And IfNull(Isd.IsRequired_LotNo,0) <> 0 Then S.LotNo Else Null End "
            mLotNoFieldName = " Case When  " & IsGroupOnLotNo & " = 1 AND IfNull(Isd.IsRequired_LotNo,0) <> 0 Then S.LotNo Else Null End "
            ...
            Else
                ...
                If AgL.StrCmp(ReportFrm.FGetText(6), "Not Zero") Then
                    strCond = " Having Round(IfNull(Sum(S.Qty_Rec),0) - IfNull(Sum(S.Qty_Iss),0),4) <> 0 "
                ElseIf AgL.StrCmp(ReportFrm.FGetText(6), "Greater Than Zero") Then
                    strCond = " Having Round(IfNull(Sum(S.Qty_Rec),0) - IfNull(Sum(S.Qty_Iss),0),4) > 0 "
                ElseIf AgL.StrCmp(ReportFrm.FGetText(6), "Less Than Zero") Then
                    strCond = " Having Round(IfNull(Sum(S.Qty_Rec),0) - IfNull(Sum(S.Qty_Iss),0),4) < 0 "
                ...
                If IsGroupOnLotNo = 1 Then
                    ...
                    mItemQry = " Select S.SubCode ,  S.Item , IfNull(S.Dimension1,'') AS Dimension1, IfNull(S.Dimension2,'') AS Dimension2, " & _
                               " " & mLotNoFieldName & " As LotNo " & _
                    ...
                    mParentQry = " (" & mItemQry & ") As VItem " & _
                                 " LEFT JOIN StockProcess S ON S.Item = VItem.Item AND S.SubCode = VItem.SubCode AND IfNull( S.LotNo ,'') = IfNull(VItem.LotNo,'') AND IfNull(S.Dimension1,'') = IfNull(VItem.Dimension1,'') AND IfNull(S.Dimension2,'') = IfNull(VItem.Dimension2,'') "
                ...
                Else
                    ...
                    mParentQry = " (" & mItemQry & ") As VItem LEFT JOIN StockProcess S ON S.Item = VItem.Item AND S.SubCode = VItem.SubCode AND IfNull(S.Dimension1,'') = IfNull(VItem.Dimension1,'') AND IfNull(S.Dimension2,'') = IfNull(VItem.Dimension2,'') "
            ...
            StrOpening = " SELECT S.Site_Code, S.SubCode, S.Process," & _
                        ...
                        " S.Item ,S.Dimension1,S.Dimension2, Max(I.Unit) AS Unit," & _
                        " " & mLotNoFieldName & " As LotNo, " & _
                        " Sum(IfNull(S.Qty_Rec,0)) - Sum(IfNull(S.Qty_Iss,0)) AS Opening, 0 AS ReceiveQty, 0 AS IssueQty, " & _
                        " Sum(IfNull(S.Measure_Rec,0)) - Sum(IfNull(S.Measure_Iss,0)) AS OpeningMeasure, 0 AS ReceiveMeasure, 0 AS IssueMeasure, " & _
                        " 0 AS Sr " & _
                        " FROM " & mParentQry & " " & _
                        " LEFT JOIN Item I On S.Item = I.Code " & _
                        " LEFT JOIN SubGroup Sg On S.SubCode = Sg.SubCode " & _
                        ...
                        " Having Round(Sum(IfNull(S.Qty_Rec,0)) - Sum(IfNull(S.Qty_Iss,0)),4) <> 0 "
            ...
            StrReceive = " SELECT S.Site_Code, S.SubCode, S.Process, " & _
                    ...
                    " S.Item ,S.Dimension1,S.Dimension2, I.Unit AS Unit, " & _
                    " " & mLotNoFieldName & " As LotNo, " & _
                    " 0 AS Opening,IfNull(S.Qty_Rec,0)AS ReceiveQty,0 AS IssueQty,   " & _
                    " 0 AS OpeningMeasure,IfNull(S.Measure_Rec,0) AS ReceiveMeasure,IfNull(S.Measure_Iss,0) AS IssueMeasure,   " & _
                    " 1 AS Sr " & _
                    " FROM " & mParentQry & " " & _
                    " LEFT JOIN Item I On S.Item = I.Code " & _
                    " LEFT JOIN SubGroup Sg On S.SubCode = Sg.SubCode " & _
            ...
            StrIssue = " SELECT S.Site_Code, S.SubCode, S.Process, " & _
                    ...
                    " S.Item ,S.Dimension1,S.Dimension2, I.Unit AS Unit, " & _
                    " " & mLotNoFieldName & " As LotNo, " & _
                    " 0 AS Opening,0 AS ReceiveQty,IfNull(S.Qty_Iss,0) AS IssueQty,   " & _
                    " 0 AS OpeningMeasure,IfNull(S.Measure_Rec,0)AS ReceiveMeasure,IfNull(S.Measure_Iss,0) AS IssueMeasure,   " & _
                    " 2 AS Sr " & _
                    " FROM " & mParentQry & " " & _
                    " LEFT JOIN Item I On S.Item = I.Code " & _
            ...
            If AgL.StrCmp(ReportFrm.FGetText(2), "Voucher Type Wise Summary") Then
                ...
                mQry = " SELECT   V1.Process, V1.Unit As Unit1, Max(U.DecimalPlaces) AS  Unit1DecimalPlace,  max(Case When " & IsGroupOnJobWorker & " = 1 THEN V1.JobWorker ELSE NULL END ) AS JobWorker, max(Case When " & IsGroupOnProcess & " = 1 THEN P.Description ELSE NULL END ) AS ProcessDesc, " & _
                        " V1.Sr, max(V1.Rec_V_Type) AS Rec_V_Type, sum(IfNull(V1.Rec_Qty,0)) AS Rec_Unit1, max(V1.Iss_V_Type) AS Iss_V_Type, sum(IfNull(V1.Iss_Qty,0)) AS Iss_Unit1, " & _
    ...
    Private Sub ProcStockBalance()
        ...
        Try
            ...
            If ReportFrm.FGetText(1) = "Measure" Then
                ...
                Unit1 = "I.MeasureUnit"
                Unit1DecimalPlace = "MU.DecimalPlaces"
                BalanceUnit1 = "Sum(IfNull(S.Measure_Rec,0)) - Sum(IfNull(S.Measure_Iss,0))"
            Else
                Unit1Head = "'Qty'"
                Unit1 = "I.Unit"
                Unit1DecimalPlace = "U.DecimalPlaces"
                BalanceUnit1 = "Sum(IfNull(S.Qty_Rec,0)) - Sum(IfNull(S.Qty_Iss,0))"
            End If
            ...
            If ReportFrm.FGetText(2) = "Process" Then
                mGroupOn1Value = "IfNull(P.StockHead,P.Description)"
            ...
            If AgL.StrCmp(ReportFrm.FGetText(4), "Greater Than Zero") Then
                mBalcondStr = " Having Round(IfNull(Sum(S.Qty_Rec),0),4) - Round(IfNull(Sum(S.Qty_Iss),0),4) > 0 "
            ...
            mLotNoFieldName = "Case When " & IsGroupOnLotNo & " = 1 And IfNull(max(convert(INT,Isd.IsRequired_LotNo)),0) <> 0 Then max(S.LotNo) Else Null End "
    ...
    Private Sub ProcStockBalanceValuation()
        ...
        Try
            ...
            mQry = "SELECT VMain.*, I.Unit, U.DecimalPlaces, IG.Description AS ItemGroup, " & _
                    ...
                    " FROM (  " & _
                    " SELECT I.Code AS Item, max(I.Description) AS ItemName, IfNull(sum(H.Qty_Rec),0)- IfNull(sum(H.Qty_Iss),0) AS BalQty " & _
                    " FROM Stock H " & _
                    " LEFT JOIN Item I ON I.Code = H.Item  " & _
                    " Where H.V_Date <= " & AgL.Chk_Text(ReportFrm.FGetText(0)) & "  " & mStockCondStr & _
                    " GROUP BY I.Code  " & _
                    " Having IfNull(sum(H.Qty_Rec),0)- IfNull(sum(H.Qty_Iss),0) > 0 " & _
                    " ) As p   " & _
                    " LEFT JOIN  " & _
                    " (  " & _
                    ...
                    " FROM Stock H " & _
                    " LEFT JOIN Item I ON I.Code = H.Item  " & _
                    " WHERE IfNull(H.Qty_Rec,0) <> 0 AND H.V_Type in ('GR','PINV')" & _
                    " AND H.V_Date <= " & AgL.Chk_Text(ReportFrm.FGetText(0)) & "  " & mStockCondStr & _
                    " )  s   " & _
                    " ) SD ON SD.Item = p.Item " & _
                    " AND IfNull(p.BalQty,0)  > IfNull(SD.sum_stock,0)  - IfNull(SD.Qty,0) " & _
                    " ) VMain " & _
...
#Region "Stock Balance With Average Rate"
    Private Sub ProcStockBalanceWithAverageRate()
        ...
        Try
            ...
            'mQry = " SELECT V1.*, VInv.AvgRate, IfNull(VInv.AvgRate,0)*V1.BalUnit1 AS AvgAmount, SM.Name AS Site_Name, G.Description AS GodownDesc,  P.Description AS ProcessDesc, I.Description AS ItemDesc, " & _
            '       " " & Unit1Head & " AS Unit1Head, " & Unit1DecimalPlace & " AS Unit1DecimalPlace, " & Unit1 & " AS Unit1, " & _
            '       " '" & ReportFrm.FGetText(2) & "' As GroupOn1Head, " & mGroupOn1Value & " AS GroupOnValue1, " & _
            '       " " & mShowForHead1 & " AS mShowForHead1, " & mShowForHead2 & " AS mShowForHead2, " & mShowForHead3 & " AS mShowForHead3, " & _
            '       " " & mShowForValue1 & " AS mShowForValue1, " & mShowForValue2 & " AS mShowForValue2, " & mShowForValue3 & " AS mShowForValue3 " & _
            ...
            mQry = " SELECT V1.*, VInv.PurchRate, IfNull(VInv.PurchRate,0)*V1.BalUnit1 AS PurchAmount, SM.Name AS Site_Name, G.Description AS GodownDesc,  P.Description AS ProcessDesc, I.Description AS ItemDesc, " & _
           " " & Unit1Head & " AS Unit1Head, " & Unit1DecimalPlace & " AS Unit1DecimalPlace, " & Unit1 & " AS Unit1, " & _
           " '" & ReportFrm.FGetText(2) & "' As GroupOn1Head, " & mGroupOn1Value & " AS GroupOnValue1, " & _
           " " & mShowForHead1 & " AS mShowForHead1, " & mShowForHead2 & " AS mShowForHead2, " & mShowForHead3 & " AS mShowForHead3, " & _
           " " & mShowForValue1 & " AS mShowForValue1, " & mShowForValue2 & " AS mShowForValue2, " & mShowForValue3 & " AS mShowForValue3 " & _
