...
#End Region
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = " SELECT H.DocID AS SearchCode, H.V_Type AS [Issue_Type], H.V_Date AS Date, " &
                ...
                " GF.Description as To_Godown, H.Remarks,   P.Description as Process, " &
    ...
    Private Sub FrmProductionOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SQLiteConnection, ByVal Cmd As SQLiteCommand) Handles Me.BaseEvent_Save_InTrans
        ...
        For I = 0 To Dgl1.RowCount - 1
            If Dgl1.Item(Col1Item, I).Value <> "" Then
                mSr += 1
                mQry = " INSERT INTO dbo.StockHeadDetail ( DocID, Sr, Item_UID, Item, Dimension1, Dimension2, Specification, Manufacturer, LotNo, BaleNo, Godown, Qty, Unit, " &
        ...
        mQry = " INSERT INTO Stock (DocId, Sr, V_Type, V_Prefix,  V_Date, V_No, RecID, Div_Code, Site_Code, SubCode, " &
                ...
                " sum(L.Amount) AS Amount, sum(L.Amount) AS Amount, 'R', max(L.CostCenter) AS Cost, L.LotNo, L.BaleNo, L.Process, max(H.Remarks) AS Remarks, H.DocID, row_number() OVER (ORDER BY L.Item) " &
    ...
    Private Sub FPostInJobIssRecUID(ByVal SearchCode As String, ByVal Conn As SQLiteConnection, ByVal Cmd As SQLiteCommand)
        ...
        mQry = " INSERT INTO JobIssRecUID(DocID, TSr, Sr, IssRec, Process, JobRecDocID, Item, Item_UID, " &
                 ...
                 " Select L.DocId, L.Sr As TSr, L.Sr, 'R', H.Process, JIR.DocId, L.Item, L.Item_Uid, " &
        ...
        mQry = " Update JobIssRecUID " &
                ...
                " And Item_UID In (Select Item_UID From StockHeadDetail  Where DocId = '" & mSearchCode & "' And Item_Uid Is Not Null) " &
    ...
    Private Sub FrmProductionOrder_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        With DsTemp.Tables(0)
            If .Rows.Count > 0 Then
                ...
                mQry = "Select S.Item_UID, S.Item, S.Specification, S.Qty, S.Unit, S.MeasurePerPcs, " &
                       "S.TotalMeasure, S.MeasureUnit, S.Rate, S.Amount, S.LotNo, S.BaleNo, S.Process, " &
                       "S.Remarks, S.CurrentStock, S.Manufacturer, Iu.Item_Uid As Item_UidDesc, " &
    ...
    Public Function FDataValidation_Item_UID() As String
        ...
        mQry = " SELECT Sg.DispName, H.ManualRefNo, H.V_Date, P.Description AS ProcessDesc, Iu.Item_Uid As Item_UidDesc " &
                " FROM JobIssRecUID L  " &
                " LEFT JOIN StockHead H  ON L.JobRecDocID = H.DocID  " &
                " LEFT JOIN SubGroup Sg   ON H.SubCode = Sg.SubCode " &
                " LEFT JOIN Process P ON H.Process =  P.NCat " &
                ...
                " AND L.Item_Uid In (" & mItem_UidStr & ") " &
    ...
    Private Sub Validating_Item_Uid(ByVal Item_Uid As String, ByVal mRow As Integer)
        ...
        Try
            mQry = " SELECT I.Code, I.Description, I.Unit, I.ManualCode, I.Prod_Measure, I.MeasureUnit, " &
                   " U.DecimalPlaces as QtyDecimalPlaces, MU.DecimalPlaces as MeasureDecimalPlaces, IG.Description AS ItemGroupDesc, UI.Code as ItemUIDCode, UI.ProdOrder as ProdOrderDocID, PO.V_Type + '-' + PO.ManualRefNo as ProdOrderNo  " &
    ...
    '    '       " FROM (SELECT DocID, Item_UID FROM JobIssRecUID  " & _
    '    '       " WHERE Item_UID ='" & Item_UidCode & "' And IssRec= 'R') I " & _
    '    '       " LEFT JOIN JobIssRecUID R  ON I.DocID = R.JobRecDocID AND I.Item_UID = R.Item_UID  " & _
    ...
    '    '            " WHERE Item_UID ='" & Item_UidCode & "' And IssRec='R') I " & _
    '    '            " LEFT JOIN JobIssRecUID R  ON I.DocID = R.JobRecDocID AND I.Item_UID = R.Item_UID  " & _
    '    '            " LEFT JOIN JobOrder H  ON I.DocID = H.DocID " & _
