...
#End Region
    ...
    Private Sub FrmStoreIssue_BaseEvent_ApproveDeletion_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_ApproveDeletion_InTrans
        Dim I As Integer = 0
    ...
    Private Sub FrmProductionOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SQLiteConnection, ByVal Cmd As SQLiteCommand) Handles Me.BaseEvent_Save_InTrans
        ...
        mQry = "UPDATE StockHead " &
                ...
                " TotalMeasure = " & Val(LblTotalMeasureValue.Text) & ", " &
                " Amount = " & Val(LblTotalAmountValue.Text) & ", " &
        ...
        For I = 0 To Dgl1.RowCount - 1
            If Dgl1.Item(Col1Item, I).Value <> "" Then
                ...
                mQry = " INSERT INTO dbo.StockHeadDetail ( DocID, Sr, Item_UID, Item, Process, Dimension1, Dimension2, Specification, LotNo, BaleNo, Godown, Qty, Unit, " &
        ...
        '                " From StockHead As H   " & _
        '                " LEFT JOIN StockHeadDetail As L  On H.DocId = L.DocId " & _
        ...
        mQry = " INSERT INTO Stock (DocId, Sr, V_Type, V_Prefix,  V_Date, V_No, RecID, Div_Code, Site_Code, SubCode, " &
                " Item, Dimension1, Dimension2, Manufacturer, Godown, Qty_Iss, Unit,  MeasurePerPcs, Measure_Iss, MeasureUnit,  Rate, Amount, Landed_Value, EType_IR, " &
                " Cost, LotNo, BaleNo, Process, Remarks, ReferenceDocId, ReferenceDocIdSr)  " &
                " SELECT H.DocID, row_number() OVER (ORDER BY L.Item), max(H.V_Type) AS V_Type, max(H.V_Prefix) AS V_Prefix, max(H.V_Date) AS V_Date, max(H.V_No) AS V_No, Max(H.ManualRefNo) AS RecId, " &
                ...
                " sum(L.Amount) AS Amount, sum(L.Amount) AS Amount, 'I', max(L.CostCenter) AS Cost, L.LotNo, L.BaleNo, L.Process, max(H.Remarks) AS Remarks, H.DocID, row_number() OVER (ORDER BY L.Item) " &
        ...
        If CType(AgL.VNull(DtV_TypeSettings.Rows(0)("IsPostedInStockProcess")), Boolean) Then
            mQry = " INSERT INTO StockProcess (DocId, Sr, V_Type, V_Prefix,  V_Date, V_No, RecID, Div_Code, Site_Code, SubCode, " &
                    " Item, Dimension1, Dimension2, Manufacturer, Godown, Qty_Rec, Unit,  MeasurePerPcs, Measure_Rec, MeasureUnit,  Rate, Amount, " &
                    " Cost, LotNo, BaleNo, Process, Remarks, ReferenceDocId, ReferenceDocIdSr)  " &
                    " SELECT H.DocID, row_number() OVER (ORDER BY L.Item), max(H.V_Type) AS V_Type, max(H.V_Prefix) AS V_Prefix, max(H.V_Date) AS V_Date, max(H.V_No) AS V_No, Max(H.ManualRefNo) AS RecId, " &
                    " max(H.Div_Code) AS Div_Code, max(H.Site_Code) AS Site_Code, max(H.SubCode) AS SubCode, L.Item, L.Dimension1, L.Dimension2, L.Manufacturer, max(H.FromGodown) AS Godown, " &
                    ...
                    " sum(L.Amount) AS Amount, max(L.CostCenter) AS Cost, L.LotNo, L.BaleNo, H.Process, max(H.Remarks) AS Remarks, H.DocID, row_number() OVER (ORDER BY L.Item) " &
                    " FROM StockHeadDetail L " &
                    " LEFT JOIN StockHead H ON H.DocID = L.DocID " &
                    " WHERE H.DocID = " & AgL.Chk_Text(mSearchCode) & " " &
    ...
    Private Sub FPostInJobIssRecUID(ByVal SearchCode As String, ByVal Conn As SQLiteConnection, ByVal Cmd As SQLiteCommand)
        ...
        mQry = " INSERT INTO JobIssRecUID(DocID, TSr, Sr, IssRec, Process, Item, Item_UID, " &
                 ...
                 " H.FromGodown, H.Site_Code, H.V_Date, H.V_Type, H.SubCode, H.Div_Code, H.ManualRefNo, H.EntryDate, " &
                 " SubString(IfNull(H.Remarks,'') + '.' + IfNull(L.Remarks,''),0,255) " &
                 " From (Select * From StockHeadDetail  Where DocId = '" & mSearchCode & "' And Item_Uid Is Not Null) As L " &
    ...
    Private Sub FrmProductionOrder_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        mQry = "Select H.*, G.Description as FromGodownDesc, P.Description as ProcessDesc, R.Description As ReasonDesc, " &
        ...
        With DsTemp.Tables(0)
            If .Rows.Count > 0 Then
                ...
                mQry = "Select S.*, I.ManualCode as Item_No, Iu.Item_Uid As Item_UidDesc, I.Description as Item_Desc, R.ReferenceNo AS ReqNo, " &
                       "U.DecimalPlaces as QtyDecimalPlaces, MU.DecimalPlaces as MeasureDecimalPlaces, IG.Description AS ItemGroupDesc," &
                       "P.Description as ProcessDesc, Cm.Name As CostCenterName,  " &
                       ...
                       "LEFT JOIN Item_Uid Iu ON S.Item_Uid = Iu.Code " &
                       ...
                       "Left Join CostCenterMast Cm On S.CostCenter = Cm.Code " &
                       "Order By Sr"
                DsTemp = AgL.FillData(mQry, AgL.GCn)
                With DsTemp.Tables(0)
                    Dgl1.RowCount = 1
        ...
        If DtTemp.Rows.Count > 0 Then
            For I = 0 To DtTemp.Rows.Count - 1
                mQry = "SELECT Sg.Name, H.ManualRefNo, H.V_Date, Vc.NCatDescription AS ProcessDesc, " &
                            ...
                            " FROM (SELECT DocID, Item_UID FROM JobIssRecUID  " &
    ...
    Private Sub Validating_Item_Uid(ByVal Item_Uid As String, ByVal mRow As Integer)
        ...
        Try
            mQry = " SELECT I.Code, I.Description, I.Unit, I.ManualCode, I.Prod_Measure, I.MeasureUnit, I.Rate, IG.Description AS ItemGroupDesc," &
    ...
    Private Sub FCreateHelpSubgroup()
        ...
        If TxtProcess.Text = "" Then
            mQry = " SELECT H.SubCode AS Code, H.Name + (Case When IfNull(H.CityCode,'')<>'' Then ', ' + C.CityName Else '' End) as Name  " &
                    " FROM Subgroup H   " &
        ...
        Select Case ColumnName
            Case Col1Item
                If RbtIssueForReqisition.Checked Then
                    mQry = " SELECT max(L.Item) AS Item, max(I.Description) AS ItemDesc, max(I.ManualCode) AS ItemCode, max(H.ReferenceNo) AS RequisitionNo, IfNull(sum(L.ApproveQty),0) - IfNull(sum(V1.IssueQty),0) AS Qty, " &
                            " max(L.Unit) AS Unit, Max(IG.Description) AS ItemGroupDesc, H.DocID AS RequisitionDocId, L.Sr AS RequisitionSr,  max(L.MeasurePerPcs) AS MeasurePerPcs, max(I.Rate) AS Rate, " &
                            " IfNull(Max(U.DecimalPlaces),0) As QtyDecimalPlaces, IfNull(Max(UM.DecimalPlaces),0) As MeasureDecimalPlaces, max(L.MeasureUnit) AS MeasureUnit, " &
                            " NULL AS ProcessCode, NULL AS Dimension1, NULL AS Dimension2, Null AS LotNo, NULL AS " & AgTemplate.ClsMain.FGetDimension1Caption() & ", NULL AS " & AgTemplate.ClsMain.FGetDimension2Caption() & ", NULL AS Process, " &
                            " '" & RbtIssueForReqisition.Text & "' AS V_Nature " &
                            " FROM Requisition H " &
                            " LEFT JOIN RequisitionDetail L ON L.DocId = H.DocID " &
            ...
            Case Col1ItemCode
                If RbtIssueForReqisition.Checked Then
                    mQry = " SELECT max(L.Item) AS Code, max(I.ManualCode) AS ItemCode, max(I.Description) AS ItemDesc,  max(H.V_Type) + '-' + max ( Convert(NVarchar,H.V_No)) AS PlanningNo, L.MaterialPlanSr , L.MaterialPlan ,  max(L.Unit) AS Unit, " &
                            " max(L.MeasurePerPcs) AS MeasurePerPcs, Max(IG.Description) AS ItemGroupDesc, max(L.MeasureUnit) AS MeasureUnit, IfNull(sum(L.UserPurchPlanQty ),0) - IfNull(sum(D.IndQty ),0) AS PlanQty, sum(L.UserPurchPlanMeasure ) - IfNull(sum(D.IndMeasure ),0) AS PlanMeasure, " &
    ...
    Private Sub ProcFillRequisitionDetails(ByVal bRequisitionStr As String)
        ...
        Try
            ...
            mQry = " SELECT H.DocID, L.Sr, max(H.ReferenceNo) AS ReqNo, IfNull(sum(L.ApproveQty),0) - IfNull(sum(V1.IssueQty),0) AS BalQty, " &
                    " max(L.Item) AS Item, max(I.Description) AS ItemDesc, max(I.ManualCode) AS ItemCode  ,max(L.Unit) AS Unit, max(L.MeasurePerPcs) AS MeasurePerPcs, " &
                    " IfNull(Max(U.DecimalPlaces),0) As QtyDecimalPlaces, IfNull(Max(UM.DecimalPlaces),0) As MeasureDecimalPlaces, max(L.MeasureUnit) AS MeasureUnit, '" & RbtIssueForReqisition.Text & "' AS V_Nature " &
                    " FROM Requisition H " &
                    " LEFT JOIN RequisitionDetail L ON L.DocId = H.DocID " &
                    " LEFT JOIN Item I ON I.Code = L.Item  " &
                    " LEFT JOIN Unit U ON U.Code = L.Unit  " &
                    ...
                    " AND H.Div_Code = '" & TxtDivision.Tag & "'  AND H.Site_Code ='" & TxtSite_Code.Tag & "'   " &
                    " AND H.V_Date <= '" & TxtV_Date.Text & "' AND H.RequisitionBy = '" & TxtParty.Tag & "'  " &
                    " AND L.DocId IN ( " & bRequisitionStr & " ) " & bCondStr &
                    " GROUP BY H.DocID, L.Sr  " &
                    " HAVING IfNull(sum(L.ApproveQty),0) - IfNull(sum(V1.IssueQty),0) > 0 "
    ...
    Private Function FHPGD_Items() As String
        ...
        mQry = "  SELECT 'o' As Tick,  VMain.CodeStr, VMain.ItemDesc, VMain.LotNo, VMain.ItemGroupDesc, VMain.Dimension1Desc, VMain.Dimension2Desc, VMain.Process, VMain.Qty " &
                " FROM ( " & FRetFillItemWiseQry(" ", "") & " ) As VMain " &
    ...
    Private Sub FFillItems(ByVal bItemStr As String)
        ...
        Try
            ...
            mQry = FRetFillItemWiseQry("", " And IfNull(H.Item,'') + IfNull(H.LotNo,'') + IfNull(H.Process,'') + IfNull(H.Dimension1,'') + IfNull(H.Dimension2,'') In (" & bItemStr & ")")
    ...
    Private Function FRetFillItemWiseQry(ByVal HeaderConStr As String, ByVal LineConStr As String) As String
        FRetFillItemWiseQry = " SELECT IfNull(H.Item,'') + IfNull(H.LotNo,'') + IfNull(H.Process,'') + IfNull(H.Dimension1,'') + IfNull(H.Dimension2,'') AS CodeStr, IfNull(H.Item,'') AS Item , Max(I.Description) AS ItemDesc, H.LotNo, " &
                            " Max(D1.Description) AS Dimension1Desc, Max(D2.Description) AS Dimension2Desc, Max(P.Description) AS Process, Round(IfNull(sum(H.Qty_Rec),0) - IfNull(sum(H.Qty_Iss),0),4) AS Qty, " &
                            " H.process AS ProcessCode, Max(I.Unit) AS Unit, H.Dimension1, H.Dimension2, Max(I.ManualCode) AS ItemCode, " &
                            " Max(I.Measure) AS MeasurePerPcs, Max(I.MeasureUnit) AS MeasureUnit, Max(U.DecimalPlaces) AS QtyDecimalPlaces, Max(IG.Description) AS ItemGroupDesc, Max(MU.DecimalPlaces) as MeasureDecimalPlaces, Max(I.Rate) AS Rate, " &
                            " NULL AS RequisitionNo, NULL AS RequisitionDocId, NULL AS RequisitionSr, '" & RbtnForStock.Text & "' AS V_Nature " &
                            " FROM Stock H  " &
                            ...
                            " WHERE IfNull(H.Item,'') <> ''  AND H.Godown = " & AgL.Chk_Text(TxtFromGodown.Tag) & " " &
                            " AND H.V_Date <= " & AgL.Chk_Text(TxtV_Date.Text) & " AND H.DocID <> " & AgL.Chk_Text(mInternalCode) & " " &
                            " And IfNull(I.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') = '" & AgTemplate.ClsMain.EntryStatus.Active & "'  " & HeaderConStr & LineConStr &
                            " GROUP BY H.Item, H.LotNo, H.Process, H.Dimension1, H.Dimension2 " &
                            " HAVING Round(IfNull(sum(H.Qty_Rec),0) - IfNull(sum(H.Qty_Iss),0),4) > 0 "
    End Function
    ...
    Private Sub ProcImportFromExcel()
        ...
        For I = 0 To DtMain.Rows.Count - 1
            ...
            mQry = "SELECT I.Unit, I.Measure As MeasurePerPcs, " &
                    " I.MeasureUnit, I.Rate, " &
                    " U.DecimalPlaces As QtyDecimalPlaces, U1.DecimalPlaces As MeasureDecimalPlaces, I.Specification " &
                    " FROM Item I " &
                    " LEFT JOIN Unit U On I.Unit = U.Code " &
                    " LEFT JOIN Unit U1 On I.MeasureUnit = U1.Code " &
    ...
    Private Sub FImportFromTextFile()
        ...
        For I = 0 To DtTemp.Rows.Count - 1
            ...
            mQry = StrQry & " Select Process, Sku, BarCode, Max(MeasurePerPcs) As MeasurePerPcs From @TmpTable " &
                    " Where Process = '" & TxtProcess.Tag & "' And Jobworker = '" & TxtParty.Tag & "' " &
    ...
    Public Function FCheck_Item_UID(ByVal Item_UID As String) As String
        ...
        mQry = "Select ProcessSequence, " &
                "       (Select Count(*) from ProcessSequenceDetail " &
                "        Where Code = H.ProcessSequence And Process = '" & TxtProcess.Tag & "') As IterationsAllowed " &
