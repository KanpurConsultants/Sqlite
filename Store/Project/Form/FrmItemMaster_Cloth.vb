...
#Region "Designer Code"
    Private Sub InitializeComponent()
        ...
        Me.PnlCustomGrid.Location = New System.Drawing.Point(770, 325)
        Me.PnlCustomGrid.Name = "PnlCustomGrid"
        Me.PnlCustomGrid.Size = New System.Drawing.Size(144, 41)
        ...
        Me.GBOtherDetails.Location = New System.Drawing.Point(9, 278)
        ...
        Me.GBOtherDetails.Visible = False
        '
        'Label13
        '
...
#End Region
    ...
    Public Sub FrmYarn_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = "SELECT I.Code, I.ManualCode as [Item Code], I.Description [Item Description],I.Specification, " &
                        " IG.Description AS [Item Group], IC.Description AS [Item Category], IT.Name AS [Item Type], IIG.Description AS [Item Invoice Group],I.Unit " &
                        " FROM Item I " &
    ...
    Private Sub FrmYarn_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SQLiteConnection, ByVal Cmd As SQLiteCommand) Handles Me.BaseEvent_Save_InTrans
        mQry = "UPDATE Item " &
                ...
                " Gross_Weight = " & Val(TxtStdWeight.Text) & ", " &
                ...
                " IsRestricted_InTransaction = " & IIf(TxtIsRestrictedinTransaction.Text = "Yes", 1, 0) & ", " &
                " ServiceTaxYN = 'N', " &
                " StockYN = 1, " &
    ...
    Private Sub FUpdateOtherItemDetail(ByVal Conn As SQLiteConnection, ByVal Cmd As SQLiteCommand)
        mQry = "Select count(*) From ItemSiteDetail  Where Code='" & mSearchCode & "' And Div_Code = '" & AgL.PubDivCode & "' And Site_Code ='" & AgL.PubSiteCode & "' "
        If AgL.Dman_Execute(mQry, AgL.GcnRead).ExecuteScalar > 0 Then
            mQry = " UPDATE ItemSiteDetail " &
                    " SET MinimumStockLevel = " & Val(TxtMinimumStockLevel.Text) & ", " &
                    " MaximumStockLevel = " & Val(TxtMaximumStockLevel.Text) & ", " &
                    " ReOrderStockLevel = " & Val(TxtReOrderStockLevel.Text) & ", " &
                    " IsRequired_LotNo = " & IIf(TxtIsRequired_LotNo.Text = "Yes", 1, 0) & ", " &
                    " IsMandatory_UnitConversion = " & IIf(TxtIsUnitConversionMandatory.Text = "Yes", 1, 0) & ", " &
                    " PurchQtyAllowedWithoutPO = " & Val(TxtPurchQtyAllowedWithoutPO.Text) & ", " &
                    " BinLocation = " & AgL.Chk_Text(TxtBinLocation.Text) & " " &
                    " WHERE Code = '" & mSearchCode & "' " &
                    " AND Div_Code = '" & AgL.PubDivCode & "' " &
                    " AND Site_Code = '" & AgL.PubSiteCode & "' "
        Else
            mQry = " INSERT INTO ItemSiteDetail(Code,Div_Code,Site_Code,MinimumStockLevel,MaximumStockLevel, " &
                    " ReOrderStockLevel, BinLocation, IsRequired_LotNo,	IsMandatory_UnitConversion, PurchQtyAllowedWithoutPO ) " &
                    " VALUES ( '" & mSearchCode & "','" & AgL.PubDivCode & "','" & AgL.PubSiteCode & "', " &
                    " " & Val(TxtMinimumStockLevel.Text) & ", " & Val(TxtMaximumStockLevel.Text) & ", " &
                    " " & Val(TxtReOrderStockLevel.Text) & " ," & AgL.Chk_Text(TxtBinLocation.Text) & " ," &
    ...
    Private Sub FPostRateInRateList(ByVal Conn As SQLiteConnection, ByVal Cmd As SQLiteCommand)
        ...
        mQry = " INSERT INTO RateList(Code, WEF, RateType, EntryBy, EntryDate, EntryType, " &
                ...
                " VALUES (" & AgL.Chk_Text(mSearchCode) & ", " & AgL.Chk_Text(AgL.PubLoginDate) & ",	" &
        ...
        mQry = "INSERT INTO RateListDetail(Code, Sr, WEF, Item, RateType, Rate) " &
              ...
              " 0, " & AgL.Chk_Text(AgL.PubStartDate) & ", " &
        ...
        For I = 0 To DGLRateType.RowCount - 1
            If DGLRateType.Item(Col1RateType, I).Value <> "" Then
                ...
                mQry = "INSERT INTO RateListDetail(Code, Sr, WEF, Item, RateType, Rate) " &
              " VALUES (" & AgL.Chk_Text(mSearchCode) & ", " &
              " " & mSr & ", " & AgL.Chk_Text(AgL.PubStartDate) & ", " &
              " " & AgL.Chk_Text(mSearchCode) & ", " &
    ...
    Private Sub FrmQuality1_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        mQry = "Select I.*, Ig.Description As ItemGroupDesc, IC.Description As ItemCategoryDesc, B.Description As BomDesc, IIG.Description As ItemInvoiceGroupDesc, " &
                " TH.Description AS TariffHeadDesc, IT.Name AS ItemTypeName, VC.Description AS VatCommodityDesc, IfNull(V.Cnt,0) AS Cnt " &
                " From Item I " &
                " LEFT JOIN ItemGroup Ig ON I.ItemGroup = IG.Code " &
                " LEFT JOIN ItemCategory IC ON IC.Code = I.ItemCategory " &
                " LEFT JOIN ItemType IT ON IT.Code = I.ItemType " &
                " LEFT JOIN ItemInvoiceGroup IIG ON I.ItemInvoiceGroup = IIG.Code " &
                " LEFT JOIN Bom B ON I.Bom = B.Code " &
                " LEFT JOIN TariffHead TH ON TH.Code = I.TariffHead " &
        ...
        mQry = " SELECT * FROM ItemSiteDetail " &
                " WHERE Code = '" & mSearchCode & "' " &
                " AND Div_Code = '" & AgL.PubDivCode & "' " &
                " AND Site_Code = '" & AgL.PubSiteCode & "' "
        DsTemp = AgL.FillData(mQry, AgL.GCn)
