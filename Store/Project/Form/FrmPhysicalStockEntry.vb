...
Public Class FrmPhysicalStockEntry
    ...
    Protected WithEvents BtnFillSample As System.Windows.Forms.Button
    Dim mItemTypeStr$ = ""
...
#Region "Form Designer Code"
    Private Sub InitializeComponent()
        ...
        Me.BtnFillSample = New System.Windows.Forms.Button()
        ...
        Me.BtnFillPacking.Location = New System.Drawing.Point(716, 108)
        Me.BtnFillPacking.Name = "BtnFillPacking"
        Me.BtnFillPacking.Size = New System.Drawing.Size(104, 25)
        ...
        'BtnFillSample
        '
        Me.BtnFillSample.FlatStyle = System.Windows.Forms.FlatStyle.Flat
        Me.BtnFillSample.Font = New System.Drawing.Font("Verdana", 8.25!, System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, CType(0, Byte))
        Me.BtnFillSample.Location = New System.Drawing.Point(608, 108)
        Me.BtnFillSample.Name = "BtnFillSample"
        Me.BtnFillSample.Size = New System.Drawing.Size(104, 25)
        Me.BtnFillSample.TabIndex = 765
        Me.BtnFillSample.TabStop = False
        Me.BtnFillSample.Text = "Fill Sample"
        Me.BtnFillSample.UseVisualStyleBackColor = True
        '
        ...
        Me.Controls.Add(Me.BtnFillSample)
        ...
        Me.Controls.SetChildIndex(Me.BtnFillSample, 0)
...
#End Region
        ...
        With Dgl1
            For I = 0 To .RowCount - 1
                If .Item(Col1Item, I).Value <> "" Then
                    mSr += 1
                    mQry = "INSERT INTO StockHeadDetail(DocId, Sr, Item_Uid, Item, Process, " &
                            " Status, Qty, Unit, " &
                            " MeasurePerPcs, TotalMeasure, MeasureUnit,  " &
                            " Rate, Amount) " &
                            " VALUES('" & mSearchCode & "', " & Val(mSr) & ", " &
                            " " & AgL.Chk_Text(.Item(Col1Item_Uid, I).Tag) & ", " &
            ...
            Select Case Dgl1.Columns(Dgl1.CurrentCell.ColumnIndex).Name
                ...
                Case Col1Process
                    If Dgl1.AgHelpDataSet(Col1Process) Is Nothing Then
                        If e.KeyCode <> Keys.Enter Then
                            mQry = "Select P.NCat As Code, P.Description As Process, P.Div_Code " &
                                     " From Process P "
    ...
    Private Sub Validating_Item_Uid(ByVal Item_Uid As String, ByVal mRow As Integer)
        ...
        Try
            ...
            mQry = " SELECT Iu.Code As Item_UidCode, Iu.Item_UID , Iu.Item AS ItemCode, I.Description AS Item, " &
                    " I.Unit, I.MeasureUnit, I.Prod_Measure As MeasurePerPcs " &
                    " FROM Item_UID Iu  " &
    ...
    Public Function FCheck_Item_UID(ByVal Item_UID As String, ByVal mRowIndex As Integer, ByVal BlankItem_Uid As Boolean) As String
        ...
        mQry = "SELECT Count(I.DocID) " &
               ...
               " WHERE Item_UID ='" & Dgl1.Item(Col1Item_Uid, mRowIndex).Tag & "' And IssRec= 'I') I " &
        ...
        If AgL.VNull(AgL.Dman_Execute(mQry, AgL.GcnRead).ExecuteScalar) > 0 Then
            mQry = "SELECT Sg.Name, H.ManualRefNo, H.V_Date, Vc.NCatDescription AS ProcessDesc " &
                    ...
                    " WHERE Item_UID ='" & Dgl1.Item(Col1Item_Uid, mRowIndex).Tag & "' And IssRec='I') I " &
                    " LEFT JOIN JobIssRecUID R  ON I.DocID = R.JobRecDocID AND I.Item_UID = R.Item_UID  " &
                    " LEFT JOIN JobIssRec H  ON I.DocID = H.DocID " &
    ...
    Private Sub FImportFromTextFile()
        ...
        Do
            ...
            If Line IsNot Nothing Then
                ...
                Dgl1.Item(Col1Process, I - 1).Tag = AgL.XNull(AgL.Dman_Execute("Select Process From ProcessCode  Where Code = '" & strArr(3) & "'", AgL.GcnRead).ExecuteScalar)
                Dgl1.Item(Col1Process, I - 1).Value = AgL.XNull(AgL.Dman_Execute("Select Description From Process  Where NCat = '" & Dgl1.Item(Col1Process, I - 1).Tag & "'", AgL.GcnRead).ExecuteScalar)
    ...
    Private Sub FrmDyeingIssue_BaseEvent_Topctrl_tbPrn(ByVal SearchCode As String) Handles Me.BaseEvent_Topctrl_tbPrn
        mQry = "SELECT H.DocID, H.V_Date, H.ManualRefNo, L.Item, I.Description AS ItemDesc, " &
                " L.Item_UID AS Item_UIDCode, Iu.Item_UID AS Item_UIDDesc, IfNull(L.Qty,0) As Qty, L.Unit, IfNull(L.TotalMeasure,0) As TotalMeasure  " &
                " FROM StockHead H  " &
    ...
    Private Sub FGetPrevProcess(ByVal mRow As Integer)
        ...
        mQry = " SELECT P.NCat As ProcessCode, P.Description As ProcessDesc " &
                ...
                " ORDER BY H.V_Date DESC, P.Sr DESC Limit 1"
    ...
    Private Sub BtnFillPacking_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnFillPacking.Click
        ...
        If TxtV_Type.Tag = "CPSTK" Then
            mCondStr = "And I.ItemType = 'Carpet Sku'"
        Else
            mCondStr = "And I.ItemType <> 'Carpet Sku'"
        End If
        ...
        mQry = " SELECT L.*, Iu.item_Uid As Item_UidDesc, I.Description As ItemDesc, P.Description As ProcessDesc  " &
                " FROM (SELECT * FROM JobIssRec  WHERE V_Type = 'PKREC' And V_Date >= '01/Jan/2015') AS H  " &
                " LEFT JOIN JobReceiveDetail L   ON H.DocID = L.DocId " &
                " LEFT JOIN Item I on l.Item = I.Code " &
                " LEFT JOIN Item_Uid Iu On L.Item_Uid = Iu.Code " &
                " LEFT JOIN Process P On L.PrevProcess = P.NCat " &
                " LEFT JOIN ( " &
                "       Select L.JobReceiveDocId " &
                " 	    FROM SaleChallanDetail L    " &
                " 	    LEFT JOIN SaleChallan H   ON L.DocId = H.DocID " &
                " 	    WHERE H.V_Date <= '" & TxtV_Date.Text & "' " &
                " ) AS VChallan ON L.DocId = VChallan.JobReceiveDocId  " &
                " WHERE VChallan.JobReceiveDocId IS NULL " &
                " And L.EntryDate <= '13/Sep/2015 11:08:00 PM' " &
                " And H.Godown = '" & TxtGodown.Tag & "'" & mCondStr
    ...
    Private Sub BtnFillSample_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnFillSample.Click
        Dim I As Integer = 0
        Dim DsTemp As DataSet = Nothing
        ...
        mQry = " SELECT NUll as Item_Uid, Null as Item_UidDesc, L.Item, Max(I.Description) as Itemdesc, " &
                " L.Process AS PrevProcess, Max(P.Description) AS ProcessDesc, " &
                " L.Item, IfNull(Sum(L.Qty_Rec),0) - IfNull(Sum(L.Qty_Iss),0) AS Qty, Max(I.Unit) as Unit, " &
                " Max(I.Measure) as MeasurePerPcs, (IfNull(Sum(L.Qty_Rec),0) - IfNull(Sum(L.Qty_Iss),0)) * Max(I.Measure) as TotalMeasure, Max(I.MeasureUnit) as MeasureUnit " &
                " FROM Stock L  " &
                " LEFT JOIN Item I   ON L.Item = I.Code " &
                " LEFT JOIN Process P ON L.Process = P.NCat " &
                " WHERE L.Godown = '" & TxtGodown.Tag & "' " &
                " AND I.ItemType = 'Carpet Sku' " &
                " AND L.V_Date <= '" & TxtV_Date.Text & "' " &
                " AND IfNull(I.IsSample,0) <> 0 " &
