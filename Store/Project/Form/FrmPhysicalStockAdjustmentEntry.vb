...
#End Region
    ...
    Private Sub FrmSaleOrder_BaseFunction_IniGrid() Handles Me.BaseFunction_IniGrid
        ...
        With AgCL
            ...
            .AddAgTextColumn(Dgl1, Col1ReferenceDocId, 120, 0, Col1ReferenceDocId, True, True, False)
        End With
    ...
    Private Sub FrmProductionOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SQLiteConnection, ByVal Cmd As SQLiteCommand) Handles Me.BaseEvent_Save_InTrans
        ...
        With Dgl1
            For I = 0 To .RowCount - 1
                If .Item(Col1Item, I).Value <> "" Then
                    mSr += 1
                    mQry = "INSERT INTO StockHeadDetail(DocId, Sr, Item, Process, " &
                            " CurrentStock, Qty, Unit, " &
                            " MeasurePerPcs, TotalMeasure, CurrentStockMeasure, MeasureUnit,  " &
                            " DifferenceQty, DifferenceMeasure, ReferenceDocId) " &
                            ...
                            " " & Val(.Item(Col1CurrentStockMeasure, I).Value) & ", " &
                            " " & AgL.Chk_Text(.Item(Col1MeasureUnit, I).Value) & ", " &
                            " " & Val(.Item(Col1DifferenceQty, I).Value) & ", " &
                            " " & Val(.Item(Col1DifferenceMeasure, I).Value) & ", " &
                            " " & AgL.Chk_Text(.Item(Col1ReferenceDocId, I).Value) & " " &
                            " ) "
                    AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
                End If
            Next
        ...
        mQry = " UPDATE StockHead " &
                " Set StockHead.ReferenceDocID = '" & mSearchCode & "'" &
                " Where StockHead.V_Type = '" & AgL.XNull(DtV_TypeSettings.Rows(0)("FilterInclude_ContraV_Type")) & "'   " &
                " And StockHead.FromGodown = '" & TxtGodown.Tag & "' "
        AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)

        'mQry = " UPDATE StockHead " & _
        '        " Set ReferenceDocId = '" & mSearchCode & "' " & _
        '        " Where DocId In (Select Distinct ReferenceDocId From StockHeadDetail Where DocId = '" & mSearchCode & "')"
        'AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
    ...
    Private Sub FrmProductionOrder_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        mQry = "Select H.*, G.Description As GodownDesc " &
            " From StockHead H " &
        ...
        With DsTemp.Tables(0)
            If .Rows.Count > 0 Then
                ...
                mQry = "Select L.*, Iu.item_Uid As Item_UidDesc, I.Description As ItemDesc, P.Description As ProcessDesc " &
            ...
            Select Case Dgl1.Columns(Dgl1.CurrentCell.ColumnIndex).Name
                ...
                Case Col1Process
                    If Dgl1.AgHelpDataSet(Col1Process) Is Nothing Then
                        If e.KeyCode <> Keys.Enter Then
                            mQry = "Select P.NCat As Code, Vc.Description As Process, P.Div_Code " &
                                     " From Process P " &
    ...
    Private Sub FCreateHelpItem()
        ...
        mQry = "SELECT H.Code, H.Description, H.Unit, H.ItemType, H.SalesTaxPostingGroup, Measure, MeasureUnit , " &
                " IfNull(H.IsDeleted ,0) AS IsDeleted, IfNull(H.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') AS Status, " &
                " H.Div_Code, H.Unit, H.Prod_Measure As MeasurePerPcs, H.MeasureUnit " &
                " FROM Item H " &
    ...
    Private Sub Validating_Item(ByVal ItemCode As String, ByVal mRow As Integer)
        ...
        Try
            mQry = " SELECT I.Description AS ItemDesc, I.Unit, I.MeasureUnit, I.Prod_Measure As MeasurePerPcs " &
                    " FROM Item I " &
                ...
                Case TxtGodown.Name
                    If TxtGodown.AgHelpDataSet Is Nothing Then
                        If e.KeyCode <> Keys.Enter Then
                            ...
                            mQry = "SELECT Code, Description, Site_Code, IfNull(IsDeleted,0) as IsDeleted, IfNull(Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') AS Status " &
                                    " FROM Godown " &
                                    " Order By Description "
    ...
    Private Sub FrmPhysicalStockEntry_BaseEvent_ApproveDeletion_InTrans(ByVal SearchCode As String, ByVal Conn As SQLiteConnection, ByVal Cmd As SQLiteCommand) Handles Me.BaseEvent_ApproveDeletion_InTrans
        ...
        mQry = " UPDATE StockHead Set ReferenceDocId = Null Where ReferenceDocId = '" & mSearchCode & "'"
        AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
    End Sub
    ...
    Private Sub FPostInStock(ByVal Conn As SQLiteConnection, ByVal Cmd As SQLiteCommand)
        ...
        mQry = " INSERT INTO Stock (DocId, Sr, " &
                " V_Type, V_Prefix, V_Date, " &
                " V_No,   " &
                " RecID,   " &
                " Div_Code,   " &
                " Site_Code,   " &
                ...
                " MeasureUnit, " &
                " Rate, Amount, Process) " &
                " SELECT L.DocId As DocId, L.Sr, " &
                " H.V_Type, H.V_Prefix, H.V_Date, " &
                " H.V_No,   " &
                " H.V_No,   " &
                " H.Div_Code,   " &
                " H.Site_Code,   " &
                " H.FromGodown,   " &
                " L.Item, " &
                " Case When IfNull(L.DifferenceQty,0) > 0 Then  IfNull(Abs(L.DifferenceQty),0) Else 0 End As Qty_Iss, " &
                " Case When IfNull(L.DifferenceQty,0) < 0 Then  IfNull(Abs(L.DifferenceQty),0) Else 0 End As Qty_Rec, " &
                " L.Unit, L.MeasurePerPcs MeasurePerPcs, " &
                " Case When IfNull(L.DifferenceMeasure,0) > 0 Then IfNull(Abs(L.DifferenceMeasure),0) Else 0 End As Measure_Iss, " &
                " Case When IfNull(L.DifferenceMeasure,0) < 0 Then IfNull(Abs(L.DifferenceMeasure),0) Else 0 End As Measure_Rec, " &
                " L.MeasureUnit, " &
                " L.Rate, L.Amount, L.Process " &
                " FROM StockHeadDetail L  " &
    ...
    Private Function FFillPendingChallans() As String
        ...
        mQry = " SELECT DISTINCT 'o' As Tick, H.V_Date " &
                    " FROM StockHead H " &
                    " LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type " &
                    " WHERE Vt.V_Type = '" & AgL.XNull(DtV_TypeSettings.Rows(0)("FilterInclude_ContraV_Type")) & "'   " &
                    " And H.FromGodown = '" & TxtGodown.Tag & "' " &
                    " And (H.ReferenceDocID IS NULL Or H.ReferenceDocId = '" & mSearchCode & "') "

        'mQry = " SELECT DISTINCT 'o' As Tick, H.V_Date " & _
        '        " From StockHead H " & _
        '        " LEFT JOIN StockHeadDetail L ON H.DocId = L.DocId " & _
        '        " LEFT JOIN Voucher_Type Vt ON H.V-Type = Vt.V_Type " & _
    ...
    Private Sub FFillItems(ByVal bDateStr As String)
        ...
        Try
            ...
            mQry = " SELECT L.Item, Sum(L.Qty) AS TotalQty, Sum(L.TotalMeasure) AS TotalMeasure, " &
                    " Max(I.Description) As ItemDesc, Max(L.Unit) As Unit, " &
                    " Max(L.MeasureUnit) As MeasureUnit, Max(L.MeasurePerPcs) As MeasurePerPcs " &
                    " FROM StockHead H  " &
                    " LEFT JOIN StockHeadDetail L ON H.DocID = L.DocID  " &
                    " LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type " &
                    " LEFT JOIN Item I On L.Item  = I.Code " &
                    " WHERE Vt.V_Type = '" & AgL.XNull(DtV_TypeSettings.Rows(0)("FilterInclude_ContraV_Type")) & "'   " &
                    " And H.V_Date In (" & bDateStr & ")" &
                    " And H.FromGodown = '" & TxtGodown.Tag & "' " &
                    " GROUP BY L.Item " &
                    " Order By ItemDesc "
            ...
            If DtTemp.Rows.Count > 0 Then
                For I = 0 To DtTemp.Rows.Count - 1
                    ...
                    Dgl1.Item(Col1CurrentStock, I).Value = FRetItemStock(I)
    ...
    Private Sub ChkAssumeStock0IfNotFound_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ChkAssumeStock0IfNotFound.Click
        ...
        Try
            ...
            If ChkAssumeStock0IfNotFound.Checked Then
                For I = 0 To Dgl1.Rows.Count - 1
                    If Dgl1.Item(Col1Item, I).Value <> "" Then
                        If PhysicalStockItemWiseQry = "" Then
                            PhysicalStockItemWiseQry = " Select " & AgL.Chk_Text(Dgl1.Item(Col1Item, I).Tag) & " As Item, '" & Dgl1.Item(Col1Unit, I).Value & "' As Unit, " &
                ...
                Dim mItemConStr$ = ""
                If AgL.XNull(DtV_TypeSettings.Rows(0)("FilterInclude_ItemType")) <> "" Then
                    mItemConStr = " And CharIndex('|' + I.ItemType + '|','" & AgL.XNull(DtV_TypeSettings.Rows(0)("FilterInclude_ItemType")) & "') > 0 "
                End If


                ComputerStockQry = " SELECT L.Item, L.Unit, Round(IfNull(Sum(L.Qty_Rec),0) - IfNull(Sum(L.Qty_Iss),0),4) AS ComputerStock, 0 As PhysicalStock " &
                           " FROM Stock L " &
                           " LEFT JOIN Item I ON L.Item = I.Code " &
                           " WHERE L.V_Date <= '" & TxtV_Date.Text & "'   " &
                           mItemConStr &
                           " And L.Godown = '" & TxtGodown.Tag & "' " &
                           " And L.Site_Code = '" & AgL.PubSiteCode & "' " &
                           " Group By L.Item, L.Unit " &
                           " Having IfNull(Sum(L.Qty_Rec),0) - IfNull(Sum(L.Qty_Iss),0) <> 0 "

                If PhysicalStockItemWiseQry <> "" Then
                    mQry = " Select VMain.Item, VMain.Unit, Max(I.Description) As ItemDesc, " &
                            " IfNull(Sum(VMain.ComputerStock),0) As ComputerStock, IfNull(Sum(VMain.PhysicalStock),0) As PhysicalStock  " &
                            " FROM " &
                            " (" & ComputerStockQry & " " &
                            " UNION ALL " &
                            " " & PhysicalStockItemWiseQry & ") AS VMain " &
                            " LEFT JOIN Item I ON VMain.Item = I.Code " &
                            " Group By VMain.Item, VMain.Unit  " &
                            " Order By ItemDesc "
                Else
                    mQry = " Select VMain.Item, VMain.Unit, Max(I.Description) As ItemDesc, " &
                            " IfNull(Sum(VMain.ComputerStock),0) As ComputerStock, IfNull(Sum(VMain.PhysicalStock),0) As PhysicalStock  " &
                            " FROM " &
                            " (" & ComputerStockQry & " ) AS VMain " &
                            " LEFT JOIN Item I ON VMain.Item = I.Code " &
                            " Group By VMain.Item, VMain.Unit " &
                            " Order By ItemDesc "
                End If
    ...
    Private Function FRetItemStock(ByVal mRow As Integer) As Double
        Try
            mQry = " SELECT IfNull(Sum(Qty_Rec),0) - IfNull(Sum(Qty_Iss),0) " &
                    " FROM Stock " &
                    " WHERE Item = '" & Dgl1.Item(Col1Item, mRow).Tag & "' " &
                    " And V_Date <= '" & TxtV_Date.Text & "'   " &
                    " And Unit = '" & Dgl1.Item(Col1Unit, mRow).Value & "'" &
                    " And Godown = '" & TxtGodown.Tag & "' " &
