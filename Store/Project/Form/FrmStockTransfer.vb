...
#End Region
    ...
    Private Sub FrmStockTransfer_BaseEvent_ApproveDeletion_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_ApproveDeletion_InTrans
        Dim I As Integer = 0
    ...
    Private Sub FrmProductionOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Save_InTrans
                    ...
                    If Dgl1.Rows(I).Visible = True Then
                        If Dgl1.Rows(I).DefaultCellStyle.BackColor <> RowLockedColour Then
                            mQry = " UPDATE StockHeadDetail " &
                                    ...
                                    " Process = " & AgL.Chk_Text(Dgl1.Item(Col1Process, I).Tag) & ", " &
        ...
        '    mQry = " Select Div_Code From Godown G  Where G.Code = '" & TxtToGodown.AgSelectedValue & "'"
        '    mToGodownDiv_Code = AgL.XNull(AgL.Dman_Execute(mQry, AgL.GcnRead).ExecuteScalar)

        '    Dim mMaxSr As Integer = 0
        '    mMaxSr = AgL.VNull(AgL.Dman_Execute(" Select Max(Sr) From Stock  Where DocId = '" & mSearchCode & "'", AgL.GcnRead).ExecuteScalar)
    ...
    Private Sub FPostInJobIssRecUID(ByVal SearchCode As String, ByVal Conn As SQLiteConnection, ByVal Cmd As SQLiteCommand)
        ...
        mQry = " INSERT INTO JobIssRecUID(DocID, TSr, Sr, IssRec, Process, Item, Item_UID, " &
                 " Godown, Site_Code, V_Date, V_Type, SubCode, Div_Code, RecId, EntryDate, Remark, JobRecDocID) " &
                 " Select L.DocId, L.Sr As TSr, L.Sr, 'I', " &
                 " L.Process, L.Item, L.Item_Uid, " &
                 " H.FromGodown, H.Site_Code, H.V_Date, H.V_Type, H.SubCode, H.Div_Code, H.ManualRefNo, H.EntryDate, " &
                 " SubString(IfNull(H.Remarks,'') + '.' + IfNull(L.Remarks,''),0,255), L.DocId " &
        ...
        Dim mMaxSr As Integer = AgL.VNull(AgL.Dman_Execute("Select Max(L.Sr) From StockHeadDetail L  Where L.DocId = '" & mSearchCode & "'", AgL.GcnRead).ExecuteScalar)

        mQry = " INSERT INTO JobIssRecUID(DocID, TSr, Sr, IssRec, Process, Item, Item_UID, " &
                 " Godown, Site_Code, V_Date, V_Type, SubCode, Div_Code, RecId, EntryDate, Remark, JobRecDocID) " &
                 " Select L.DocId, Row_Number() Over (Order By L.Item) + " & mMaxSr & " As TSr, " &
                 " Row_Number() Over (Order By L.Item) + " & mMaxSr & " As Sr, 'R', " &
                 " L.Process, L.Item, L.Item_Uid, " &
                 " H.ToGodown, H.Site_Code, H.V_Date, H.V_Type, H.SubCode, H.Div_Code, H.ManualRefNo, H.EntryDate, " &
                 " SubString(IfNull(H.Remarks,'') + '.' + IfNull(L.Remarks,''),0,255), L.DocId  " &
                 " From (Select * From StockHeadDetail  Where DocId = '" & mSearchCode & "' And Item_Uid Is Not Null) As L " &
                 " LEFT JOIN StockHead H  On L.DocId = H.DocId "
        AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
    ...
    Private Sub FrmProductionOrder_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        mQry = "Select H.*, Fg.Description as FromGodownDesc, Tg.Description as ToGodownDesc, " &
                " Sg.Name + (Case When IfNull(Sg.CityCode,'')<>'' Then ', ' + C.CityName Else '' End) as PartyName " &
                " From StockHead H " &
                " Left Join Godown Fg on H.FromGodown = Fg.Code " &
                " Left Join Godown Tg on H.ToGodown = Tg.Code " &
        ...
        With DsTemp.Tables(0)
            If .Rows.Count > 0 Then
                ...
                mQry = "Select S.*, I.ManualCode as Item_No, I.Description as Item_Desc, " &
                       " U.DecimalPlaces as QtyDecimalPlaces, MU.DecimalPlaces as MeasureDecimalPlaces, " &
                       " P.Description as Process_Desc, IU.Item_UID as Item_UID_Desc, " &
            ...
            Case Col1Item
                If RbtnForStock.Checked Then
                    mQry = " SELECT H.Item, Max(I.Description) AS ItemDesc, H.LotNo, " &
                            ...
                            " NULL AS RequisitionNo, NULL AS RequisitionDocId, NULL AS RequisitionSr " &
                            " FROM Stock H  " &
    ...
    Private Sub FCreateHelpLotNo()
        If AgL.VNull(AgL.Dman_Execute("Select IsRequired_LotNo From ItemSiteDetail L Where Code = '" & Dgl1.Item(Col1Item, Dgl1.CurrentCell.RowIndex).Tag & "' And Div_Code = '" & AgL.PubDivCode & "' And Site_Code = '" & AgL.PubSiteCode & "' ", AgL.GcnRead).ExecuteScalar) <> 0 Then
            mQry = " SELECT L.LotNo AS Code, L.LotNo, IfNull(Sum(L.Qty_Rec), 0) - IfNull(Sum(L.Qty_Iss), 0) AS Qty " &
                    ...
                    " And L.V_Date <= '" & TxtV_Date.Text & "' " &
                    " And L.Godown = '" & TxtFromGodown.Tag & "' " &
    ...
    Private Sub FrmStoreIssue_BaseEvent_Topctrl_tbPrn(ByVal SearchCode As String) Handles Me.BaseEvent_Topctrl_tbPrn
        mQry = " SELECT H.DocID, H.V_Type, H.V_Date, H.ManualRefNo, H.Remarks, H.EntryBy, H.EntryDate, " &
                " H.ApproveBy, H.ApproveDate,  H.Status,  " &
                " L.Item, L.Sr, L.QTY, L.UNIT, L.REMARKS AS LINEREMARKS, L.LotNo,  " &
                " FG.DESCRIPTION AS FromGodownDesc, TG.Description as ToGodownDesc,  I.Description AS ItemDesc, Unit.DecimalPlaces,   " &
                ...
                " I.ITEMGROUP, I.ITEMTYPE, IG.DESCRIPTION AS ItemGroupDesc " &
                " FROM StockHead H   " &
                ...
                " Left join Unit On I.Unit = Unit.Code " &
    ...
    Private Sub FPostInStock(ByVal SearchCode As String, ByVal Conn As SQLiteConnection, ByVal Cmd As SQLiteCommand)
        ...
        mQry = " INSERT INTO Stock (DocId, Sr, V_Type, V_Prefix,  V_Date, V_No, RecID, Div_Code, Site_Code, SubCode, " &
                  ...
                  " sum(L.Amount) AS Amount, max(L.CostCenter) AS Cost, L.LotNo, L.BaleNo, L.Process, max(H.Remarks) AS Remarks, H.DocID, row_number() OVER (ORDER BY L.Item) " &
        ...
        mQry = " INSERT INTO Stock (DocId, Sr, V_Type, V_Prefix,  V_Date, V_No, RecID, Div_Code, Site_Code, SubCode, " &
              ...
              " Cost, LotNo, BaleNo, Process, Remarks, ReferenceDocId, ReferenceDocIdSr)  " &
              " SELECT H.DocID, Row_Number() Over (Order By L.Item) + " & mMaxSr & ", max(H.V_Type) AS V_Type, max(H.V_Prefix) AS V_Prefix, max(H.V_Date) AS V_Date, max(H.V_No) AS V_No, Max(H.ManualRefNo) AS RecId, " &
              " '" & mToGodownDiv_Code & "' AS Div_Code, max(H.Site_Code) AS Site_Code, max(H.SubCode) AS SubCode, L.Item, L.Dimension1, L.Dimension2, L.Manufacturer, max(H.ToGodown) AS Godown, " &
