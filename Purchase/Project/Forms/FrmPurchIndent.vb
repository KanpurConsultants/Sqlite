...
#Region "Form Designer Code"
    Private Sub InitializeComponent()
        ...
        Me.BtnFillIndentDetail.Location = New System.Drawing.Point(606, 175)
        ...
        Me.GrpDirectIndent.Size = New System.Drawing.Size(326, 28)
        ...
        Me.TxtReferenceNo.Location = New System.Drawing.Point(595, 33)
        Me.TxtReferenceNo.MaxLength = 20
        Me.TxtReferenceNo.Name = "TxtReferenceNo"
        Me.TxtReferenceNo.Size = New System.Drawing.Size(161, 18)
        ...
        Me.LblReferenceNo.Size = New System.Drawing.Size(67, 16)
        Me.LblReferenceNo.TabIndex = 740
        Me.LblReferenceNo.Text = "Indent No."
        '
        'FrmPurchIndent
        '
...
#End Region
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = " SELECT H.DocID AS SearchCode, H.DeliveryMeasure AS [Delivery Measure], H.V_Type AS [Indent Type], H.V_Prefix AS [Prefix], H.V_Date AS [Indent Date], H.ManualRefNo AS [Indent No], " &
    ...
    Private Sub FrmProductionOrder_BaseFunction_IniGrid() Handles Me.BaseFunction_IniGrid
        ...
        With AgCL
            ...
            .AddAgNumberColumn(Dgl1, Col1IndentQty, 80, 8, 3, False, Col1IndentQty, True, False, True)
    ...
    Private Sub FrmProductionOrder_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        Dim strQryPurchQuotation$ = " SELECT H.PurchIndent, H.PurchIndentsr, sum(H.Qty)  AS Qty " &
                                        ...
                                        " Where H.PurchIndent = '" & mSearchCode & "'" &
        ...
        Dim strQryPurchIndentAmend$ = " SELECT H.PurchIndent, H.PurchIndentsr, sum(H.IndentQty)  AS Qty " &
                        " FROM PurchIndentDetail H " &
                        " Where H.PurchIndent = '" & mSearchCode & "' And H.DocId <> H.PurchIndent " &
                        " GROUP BY H.PurchIndent, H.PurchIndentSr "
        ...
        mQry = " SELECT H.* , SG.DispName AS IndentorName, D.Description AS DepartmentDesc, " &
                ...
                " (Case When IfNull(PQ.Qty,0) <> 0 Or  IfNull(PO.Qty,0) <> 0 Or IfNull(Pi.Qty,0) <> 0 THEN 1 ELSE 0 END) as RowLocked " &
                " FROM " &
                " ( " &
                ...
                " Left Join ( " & strQryPurchIndentAmend & " ) As Pi On L.DocID = Pi.PurchIndent and L.Sr = PO.PurchIndentSr " &
                " Order By Sr "
    ...
    Private Sub BtnFillIndentDetail_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnFillIndentDetail.Click
        If Topctrl1.Mode = "Browse" Then Exit Sub
        If RbtIndentForPlanning.Checked = False Then Exit Sub
    ...
    Private Sub FrmPurchIndent_BaseEvent_Topctrl_tbPrn(ByVal SearchCode As String) Handles Me.BaseEvent_Topctrl_tbPrn
        mQry = " SELECT H.DocID, H.V_Type, H.V_Date, D.Description AS DepartmentDesc, SG.DispName AS IndentorName, H.Remarks, H.EntryBy, " &
                " H.ApproveBy, H.ManualRefNo, L.Sr, I.Description AS ItemDesc, L.ReqQty, L.IndentQty, L.Unit, L.Rate, L.RequireDate, L.Specification, " &
                " L.Remark AS LineRemark, U.DecimalPlaces , PO.V_Type || '-' || PO.ManualRefNo AS ProdOrderNo , MP.V_Type +'-' || MP.ManualRefNo AS PlanNo " &
                " FROM PurchIndent H " &
                " LEFT JOIN Department D ON D.Code = H.Department  " &
                " LEFT JOIN SubGroup SG ON SG.SubCode = H.Indentor  " &
                " LEFT JOIN PurchIndentDetail L ON L.DocId = H.DocID  " &
        ...
        ClsMain.FPrintThisDocument(Me, TxtV_Type.Tag, mQry, "PurchIndent_Print", "Purchase Indent")
    End Sub
    ...
    Private Sub Txt_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles TxtV_Type.Validating, TxtReferenceNo.Validating
        ...
        Try
            Select Case sender.NAME
                Case TxtV_Type.Name
                    IniGrid()
                    TxtReferenceNo.Text = AgTemplate.ClsMain.FGetManualRefNo("ManualRefNo", "PurchIndent", TxtV_Type.AgSelectedValue, TxtV_Date.Text, TxtDivision.AgSelectedValue, TxtSite_Code.AgSelectedValue, AgTemplate.ClsMain.ManualRefType.Max)
