...
#Region "Form Designer Code"
    Private Sub InitializeComponent()
        ...
        Me.GrpDirectChallan.Location = New System.Drawing.Point(93, 217)
        ...
        Me.RbtForJobReceive.Location = New System.Drawing.Point(5, 8)
        ...
        Me.BtnFillJobReceive.Location = New System.Drawing.Point(410, 225)
        Me.BtnFillJobReceive.Margin = New System.Windows.Forms.Padding(0)
        Me.BtnFillJobReceive.Name = "BtnFillJobReceive"
        Me.BtnFillJobReceive.Size = New System.Drawing.Size(38, 20)
...
#End Region
    ...
    Private Sub FrmTempRequisition_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SQLiteConnection, ByVal Cmd As SQLiteCommand) Handles Me.BaseEvent_Save_InTrans
        ...
        If bSelectionQry <> "" Then
            mQry = " INSERT INTO JobQCDetail(DocId, Sr, Dimension1, Dimension2, StockItem,  StockDimension1, StockDimension2, JobReceive, JobReceiveSr, QcQty, " &
    ...
    '                Dgl1.Item(Col1PassedQty, mRow).Value = AgL.VNull(Dgl1.AgDataRow.Cells("Bal.Qty").Value)
    ...
    Private Function FHPGD_PendingJobReceiveItems() As String
        ...
        If TxtJobWorker.Tag <> "" Then
            strCond = " AND H.JobWorker ='" & TxtJobWorker.Tag & "'"
        End If

        If TxtPartyName.Tag <> "" Then
            strCond = " AND PD.Party ='" & TxtPartyName.Tag & "'"
        End If
    ...
    Private Sub FPostInProdOrder(ByVal Conn As SQLiteConnection, ByVal Cmd As SQLiteCommand)
        ...
        mQry = "INSERT INTO ProdOrder(DocID, V_Type, V_Prefix, V_Date, V_No,Div_Code,Site_Code, " &
                " Remarks, EntryBy, EntryDate,	EntryType, " &
    ...
    Private Sub FPostStock(ByVal Conn As SQLiteConnection, ByVal Cmd As SQLiteCommand)
        mQry = " INSERT INTO Stock (DocId, Sr, V_Type, V_Prefix,  V_Date, V_No, RecID, Div_Code, Site_Code, Item, Dimension1, Dimension2, Godown, Qty_Iss, Unit, Remarks, ReferenceDocId, ReferenceDocIdSr)   " &
                " SELECT H.DocID, row_number() OVER (ORDER BY Jrd.Item), max(H.V_Type) AS V_Type, max(H.V_Prefix) AS V_Prefix, max(H.V_Date) AS V_Date, max(H.V_No) AS V_No, Max(H.ManualRefNo) AS RecId,  max(H.Div_Code) AS Div_Code, max(H.Site_Code) AS Site_Code,  Jrd.Item,  " &
                " L.Dimension1, L.Dimension2,  max(H.Godown) AS Godown,  " &
                "  sum(L.QCQty)-sum(L.PassedQty) AS Qty_Iss, Max(L.Unit) AS Unit, max(H.Remarks) AS Remarks, H.DocID, row_number() OVER (ORDER BY Jrd.Item) " &
                "  FROM JobQcDetail L " &
                "  LEFT JOIN JobQc  H ON H.DocID = L.DocID " &
                "  LEFT JOIN JobReceiveDetail Jrd  ON L.JobReceive = Jrd.DocId ANd L.JobReceiveSr = Jrd.Sr  " &
                "  WHERE H.DocID = " & AgL.Chk_Text(mSearchCode) & "   " &
                "  GROUP BY H.DocID, JRD.Item, L.Dimension1, L.Dimension2"
        AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
        ...
        mQry = " INSERT INTO Stock (DocId, Sr, V_Type, V_Prefix,  V_Date, V_No, RecID, Div_Code, Site_Code, Item, Dimension1, Dimension2, Godown, Qty_Rec, Unit, Remarks, ReferenceDocId, ReferenceDocIdSr)   " &
                " SELECT H.DocID, row_number() OVER (ORDER BY L.StockItem) + " & mMaxSr & ", max(H.V_Type) AS V_Type, max(H.V_Prefix) AS V_Prefix, max(H.V_Date) AS V_Date, max(H.V_No) AS V_No, Max(H.ManualRefNo) AS RecId,  max(H.Div_Code) AS Div_Code, max(H.Site_Code) AS Site_Code,  L.StockItem,  " &
                " L.StockDimension1, L.StockDimension2,  max(H.Godown) AS Godown,  " &
                "  sum(L.QCQty)-sum(L.PassedQty) AS Qty_Rec, Max(L.Unit) AS Unit, max(H.Remarks) AS Remarks, H.DocID, row_number() OVER (ORDER BY L.StockItem) " &
                "  FROM JobQcDetail L " &
                "  LEFT JOIN JobQc  H ON H.DocID = L.DocID " &
                "  LEFT JOIN JobReceiveDetail Jrd  ON L.JobReceive = Jrd.DocId ANd L.JobReceiveSr = Jrd.Sr  " &
                "  WHERE H.DocID = " & AgL.Chk_Text(mSearchCode) & "   " &
                "  GROUP BY H.DocID, L.StockItem, L.StockDimension1, L.StockDimension2"
        AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
    End Sub
