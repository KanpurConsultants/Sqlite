...
Public Class FrmDifferentialIncome
    ...
    Private Function Data_Validation(Optional ByVal IsSaveTimeValidation As Boolean = True) As Boolean
        ...
        Try
            ...
            If TxtDate_To.Text.Trim <> "" Then
                mCondStr = mCondStr & " And (" & AgL.Chk_Text(CDate(TxtDate_From.Text).ToString("u")) & " Between Date_From And Date_To Or " & AgL.Chk_Text(CDate(TxtDate_To.Text).ToString("u")) & " Between Date_From And Date_To) "
            End If

            mQry = "Select IfNull(Count(*),0) Cnt From DifferentialIncome " & _
                    " Where 1=1 " & mCondStr
            ...
            If IsSaveTimeValidation Then
                ...
                If Topctrl1.Mode = "Add" Then
                    If mSearchCode.Trim <> "" Then
                        AgL.ECmd = AgL.Dman_Execute("Select IfNull(count(*),0) From DifferentialIncome Where DocId='" & mSearchCode & "' ", AgL.GCn)
    ...
    Private Function AccountPosting() As Boolean
        ...
        mDifferentialIncomeAc = AgL.FillData("Select IfNull(DifferentialIncomeAc,'') From Enviro", AgL.GCn).Tables(0).Rows(0)(0)
    ...
    Private Sub BtnFillDetail_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnFillDetail.Click
        ...
        mQry = "SELECT Sum(L.BusinessVolume) AS BusinessVolume, Sum(L.PointValue ) AS PointValue " &
               "FROM SaleInvoiceDetail L  " &
               "LEFT JOIN SaleInvoice H   ON l.DocId = h.DocID " &
               "WHERE H.V_Date BETWEEN " & AgL.Chk_Text(CDate(TxtDate_From.Text).ToString("u")) & " and " & AgL.Chk_Text(CDate(TxtDate_To.Text).ToString("u")) & "  "
        DtTemp = AgL.FillData(mQry, AgL.GCn).tables(0)
        ...
        With DtTemp
            For I = 0 To .Rows.Count - 1
                If .Rows.Count > 0 Then
                    If AgL.XNull(.Rows(I)("Distributer")) = "D10216" Then
                        Debug.Print("found")
                    End If
                    ...
                    bPVPer = ClsProj.FGetPVPer(ClsProj.FGetTotalComPV(AgL.XNull(.Rows(I)("Distributer")), True, TxtDate_From.Text, TxtDate_To.Text) * 12)
                    bParentPvPer = ClsProj.FGetPVPer(ClsProj.FGetTotalComPV(AgL.XNull(.Rows(I)("ParentID")), True, TxtDate_From.Text, TxtDate_To.Text) * 12)
                    ...
                    mQry = "Update  [#" & bTemptable & "]  set " &
                    "ParentPVPer =" & bParentPvPer & ", " &
                    "SelfPV =" & bSelfPV & ", " &
                    "SelfBV =" & bSelfBV & ", " &
                    "GroupPV = " & bGroupPv & ", " &
                    "TotalPV = " & bTotalPv & ", " &
    ...
    Private Sub FillSaphireBonus()
        ...
        mQry = "Select SubCode From SubGroup Where SubGroupType = '" & ClsMain.SubGroupType.Distributer & "' And IfNull(IsInSapphireRace,'N') = 'N' "
        ...
        mQry = "Select SubCode, IfNull(IsInSaphireRace,'N') as IsInSapphireRace, ParentDistributer From Subgroup WHERE SubGroupType = '" & ClsMain.SubGroupType.Distributer & "'"
        DtSubCode = AgL.FillData(mQry, AgL.GCn)
        For I = 0 To DtSubCode.Rows.Count - 1
