...
Public Class FrmDIstributerQuery
    ...
    Private Sub ProcFillDistributerDetails()
        ...
        Try
            ...
            TxtPVPer.Text = ClsProj.FGetPVPer(ClsProj.FGetTotalComPV(TxtDistributerId.AgSelectedValue, True) * 12)
            ...
            TxtPVPerLastMonth.Text = ClsProj.FGetPVPer(ClsProj.FGetTotalComPV(TxtDistributerId.AgSelectedValue, False) * 12)
            ...
            With DtTemp
                For I = 0 To .Rows.Count - 1
                    If .Rows.Count > 0 Then
                        ...
                        bPVPer = ClsProj.FGetPVPer(ClsProj.FGetTotalComPV(AgL.XNull(.Rows(I)("Distributer")), False) * 12)
    ...
    Private Sub FillDistributorBusinessDetails(ByVal Distributer As String)
        ...
        Try
            ...
            With DtTemp
                For I = 0 To .Rows.Count - 1
                    If .Rows.Count > 0 Then
                        ...
                        bCumPV = ClsProj.FGetTotalComPV(AgL.XNull(.Rows(I)("Distributer")), True)
                        bTotalBV = ClsProj.FGetGroupBV(AgL.XNull(.Rows(I)("Distributer")), True)
                        bPVPer = ClsProj.FGetPVPer(ClsProj.FGetTotalComPV(AgL.XNull(.Rows(I)("Distributer")), True) * 12)
            ...
            mQry = "Select H.Distributer As DistributerCode, Convert(Varchar,Max(H.JoiningDate),106) As [Joining Date], Max(Sg1.ManualCode) As [Parent Distributer],  " &
                    " Max(H.DistributerId) As  [Distributer], " &
                    " Max(Space(H.Level)) + Max(H.DistributerName) As [Distributer Name] , " &
                    " Sum(H.PrevCumPV) As [Prev Cum PV] , Sum(H.SelfPV) As [Self PV] , Sum(H.GroupPV) As [Group PV], Sum(H.TotalPV) As [Total PV], " &
                    " Sum(H.PVPer) As [PV Per], Sum(H.CumPV) As [Cum PV], Sum(H.TotalBV) As [Total BV] " &
                    " From [#" & bTempTable & "] H  " &
                    " LEFT JOIN SubGroup Sg On H.Distributer Collate DATABASE_DEFAULT = Sg.SubCode " &
                    " LEFT JOIN SubGroup Sg1 On Sg.ParentDistributer = Sg1.SubCode " &
