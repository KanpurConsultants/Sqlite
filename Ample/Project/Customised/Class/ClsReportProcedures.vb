...
#Region "Reports Constant"
    ...
    Private Const SendInvoiceSMS As String = "SendInvoiceSMS"
...
#Region "Sale Report"
    ...
    Private Sub ProcSaleReportOld(ByVal HeaderTable As String, ByVal LineTable As String)
        Try
            ...
            mQry = " SELECT " & strGrpFld & " as GrpField, " & strGrpFldDesc & " as GrpFieldDesc, " & strGrpFldHead & " as GrpFieldHead, " &
                    " H.DocID, H.V_Date, " &
                    " Party.Name || ',' || IfNull(City.CityName,'') As SaleToPartyName , " &
                    " H.ReferenceNo,  " &
                    " I.Description As ItemDesc, " &
                    " L.Net_Amount, L.Qty, L.Unit " &
                    " FROM " & HeaderTable & " H " &
                    " Left Join " & LineTable & " L On H.DocID = L.DocID " &
    ...
    Private Sub ProcSendInvoiceSms()
        Try
            RepName = "Trade_SaleReport" : RepTitle = "Sale Report"
            ...
            Dim mCondStr$ = ""
            Dim mInvCount As Int16
            Dim mSmsCount As Int16
            ...
            mQry = "SELECT H.DocId,Max(sg.ManualCode) as CustomerId,  Max(substring(name,0,charindex(' ',sg.name))) as FirstName, IfNull(Max(sg.Phone),'') as Phone,Convert(nVarchar,Max(H.V_Date),106) AS InvoiceDate, Max(h.Net_Amount) AS InvoiceAmount, Sum(L.pointvalue) AS pointValue,
                    (Select IfNull(count(*),0) from saleInvoice where SaleToParty=Max(H.SaleToParty) And RowId < Max(H.RowId) ) as InvCount
                    FROM SaleInvoice h
                    LEFT JOIN SaleInvoiceDetail L ON L.DocId = h.DocID 
                    LEFT JOIN SubGroup sg ON h.SaleToParty = Sg.SubCode 
                    WHERE IsSmsSend =0
                    GROUP BY h.DocID 
                    Order By Max(H.RowId) "

            DsRep = AgL.FillData(mQry, AgL.GCn)

            If DsRep.Tables(0).Rows.Count > 0 Then
                mInvCount = DsRep.Tables(0).Rows.Count

                While i < mInvCount


                    If DsRep.Tables(0).Rows(i)("Phone").ToString.Length > 9 Then

                        If DsRep.Tables(0).Rows(i)("InvCount") > 0 Then

                            msgString = "Dear " & AgL.XNull(DsRep.Tables(0).Rows(i)("FirstName")) & ", 

Thanks for product purchase Rs." & DsRep.Tables(0).Rows(i)("InvoiceAmount").ToString & " of PV:" & DsRep.Tables(0).Rows(i)("PointValue").ToString & " on ID:" & DsRep.Tables(0).Rows(i)("CustomerId").ToString & ". We are looking forward to serve you again.
...
Welcome to the family of Ambrozia. Thanks for your first product purchase Rs." & DsRep.Tables(0).Rows(i)("InvoiceAmount").ToString & " of PV:" & DsRep.Tables(0).Rows(i)("PointValue").ToString & " 
Your ID No. is " & DsRep.Tables(0).Rows(i)("CustomerId").ToString & ". 

Regards
Ambrozia Naturals"

                        End If
                        mhttp = WebRequest.Create("http://sms.hspsms.com/sendSMS?username=AMBROZIA &message=" & msgString & "&sendername=AMBROZ&smstype=TRANS&numbers=" & DsRep.Tables(0).Rows(i)("Phone") & "&apikey=e72f2562-69be-476a-b5a2-a4ee5c882960")
                        ...
                        If Not results.StartsWith("Wrong Username or Password") Then
                            mQry = "Update SaleInvoice set IsSmsSend=1 Where DocID = '" & DsRep.Tables(0).Rows(i)("DocID") & "' "
            ...
            Err.Raise(1, , mSmsCount.ToString + " sms send out of " + mInvCount.ToString + " invoices")
    ...
    Private Sub ProcPurchaseInvoiceReport(ByVal HeaderTable As String, ByVal LineTable As String)
        ...
        Try
            ...
            mQry = " SELECT " & strGrpFld & " as GrpField, " & strGrpFldDesc & " as GrpFieldDesc, " & strGrpFldHead & " as GrpFieldHead, " &
                        ...
                        " I.Description AS ItemDesc, H.V_Date, H.ReferenceNo, Sg.DispName || ',' || IfNull(C.CityName,'') As VendorName, L.Remark " &
                        " FROM " & LineTable & " L " &
...
#End Region
    ...
    '    Dim PurchaseInOwnAcAgainstTaxInvoice$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) > 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  And H.TINNo IS NOT NULL "
    '    Dim PurchaseInOwnAcFromNonRegisteredDealer$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) > 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  And H.TINNo IS NULL "
    '    Dim PurchaseOfExcemptGoods$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) = 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "')  = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  "
    '    Dim PurchaseOfExUp$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "')  = '" & ClsMain.SalesTaxGroupPartyNature.Central & "' "

    '    Dim TotalPurchase$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 "
    ...
    '    Dim TurnOverOfPurchaseOnePer$ = "SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 1 "
    '    Dim TurnOverOfPurchaseFivePer$ = "SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 5 "
    '    Dim TurnOverOfPurchaseTwelvePer$ = "SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 12.5 "
    ...
    '    Dim TotalAddSalesTaxPointFivePerOnPurchase$ = "SELECT Str(IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Additional_Sales_Tax_Per,0) = 0.5 "
    '    Dim TotalAddSalesTaxOnePerOnPurchase$ = "SELECT Str(IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Additional_Sales_Tax_Per,0) = 1 "
    ...
    '    Dim TotalTurnOverOfPurchase$ = "SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) <> 0 "



    '    Dim SaleInOwnAcAgainstTaxInvoice$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "' AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) > 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "' And H.TINNo IS NOT NULL "
    '    Dim SaleInOwnAcFromNonRegisteredDealer$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "' AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) > 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'   And H.TINNo IS NULL"
    '    Dim SaleOfExcemptGoods$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "' AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) = 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "')  = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  "
    '    Dim SaleOfExUp$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "' AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0  And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "')  = '" & ClsMain.SalesTaxGroupPartyNature.Central & "' "

    '    Dim TotalSale$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 "
    ...
    '    Dim TurnOverOfSaleOnePer$ = "SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 1 "
    '    Dim TurnOverOfSaleFivePer$ = "SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 5 "
    '    Dim TurnOverOfSaleTwelvePer$ = "SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 12.5 "
    ...
    '    Dim TotalAddSalesTaxPointFivePerOnSale$ = "SELECT Str(IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Additional_Sales_Tax_Per,0) = 0.5 "
    '    Dim TotalAddSalesTaxOnePerOnSale$ = "SELECT Str(IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Additional_Sales_Tax_Per,0) = 1 "
    ...
    '    Dim TotalTurnOverOfSale$ = "SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) <> 0 "

    '    Dim TotalTax$ = " SELECT Str(IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category In ('" & ClsMain.Voucher_Category.Sale & "','" & ClsMain.Voucher_Category.Purchase & "')  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 "

    '    Dim NetTax$ = " Select " & _
    '            " Str((SELECT IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0) " & _
    '            " - (SELECT IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0),15,2)   "
...
#Region "Bill Wise Profitabilty"
    Private Sub ProcBillWiseProfitabilty()
        Try
            ...
            mCondStr = " Where 1 = 1 "
            mCondStr = mCondStr & " AND H.V_Date Between '" & CDate(ReportFrm.FGetText(0)) & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' "
            ...
            mQry = " SELECT H.DocID , H.V_Type, H.V_Date, H.ReferenceNo, Sg.Name As SaleToPartyName, H.SaleToParty , " &
    ...
    Private Sub ProcPartyOutstandingReport()
        ...
        Try
            ...
            mQry = "Select DocId, V_SNo, Max(RecId) As RecId, Max(V_Type) As V_Type, " &
                            ...
                            " Max(BillAmt) As BillAmt, Abs(Sum(Adjusted)) As Receipt, " &
...
#End Region
    ...
    Private Sub FWriteMainForm(ByVal xlWorkBook As Excel.Workbook)
        Dim TotalSalesAndAddSalesTaxOnPurchaseForLocal As Double = AgL.VNull(AgL.Dman_Execute("SELECT Str(IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "' ", AgL.GCn).ExecuteScalar)

        mQry = "SELECT Str(IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "' "
        ...
        Dim PurchaseInOwnAcAgainstTaxInvoice$ = AgL.VNull(AgL.Dman_Execute(" SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Sales_Tax,0) <> 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  And H.TINNo IS NOT NULL ", AgL.GCn).ExecuteScalar)
        Dim PurchaseInOwnAcFromNonRegisteredDealer$ = AgL.VNull(AgL.Dman_Execute(" SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Sales_Tax,0) <> 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  And H.TINNo IS NULL ", AgL.GCn).ExecuteScalar)
        Dim PurchaseOfExcemptGoods$ = AgL.VNull(AgL.Dman_Execute(" SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Sales_Tax,0) = 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "')  = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  ", AgL.GCn).ExecuteScalar)
        ...
        Dim SaleInOwnAcAgainstTaxInvoice$ = AgL.VNull(AgL.Dman_Execute(" SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "' AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Sales_Tax,0) <> 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "' And H.TINNo IS NOT NULL And H.SalesTaxGroupParty = 'Local' ", AgL.GCn).ExecuteScalar)


        Dim NetTax$ = AgL.VNull(AgL.Dman_Execute(" Select " &
                " Str((SELECT IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "'  And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "' ) " &
                " - (SELECT IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "'  And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "' ),15,2)   ", AgL.GCn).ExecuteScalar)
    ...
    Private Sub FWriteVatNonVat(ByVal xlWorkBook As Excel.Workbook)
        Dim VPurchaseInOwnAcAgainstTaxInvoice$ = AgL.VNull(AgL.Dman_Execute(" SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) > 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  And H.TINNo IS NOT NULL ", AgL.GCn).ExecuteScalar)
        Dim VPurchaseInOwnAcFromNonRegisteredDealer$ = AgL.VNull(AgL.Dman_Execute(" SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) > 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  And H.TINNo IS NULL ", AgL.GCn).ExecuteScalar)
        Dim VPurchaseOfExcemptGoods$ = AgL.VNull(AgL.Dman_Execute(" SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) = 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "')  = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  ", AgL.GCn).ExecuteScalar)
        Dim VPurchaseOfExUp$ = AgL.VNull(AgL.Dman_Execute(" SELECT Sum(H.Sales_Tax_Taxable_Amt) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & ReportFrm.FGetText(0) & "' And '" & ReportFrm.FGetText(1) & "' And IfNull(H.Qty,0) > 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "')  = '" & ClsMain.SalesTaxGroupPartyNature.Central & "' ", AgL.GCn).ExecuteScalar)
    ...
    Private Sub FWriteForm24_Annexure_A(ByVal xlWorkBook As Excel.Workbook)
        ...
        mQry = " SELECT Max(H.RecId) AS InvoiceNo,  Max(H.V_Date) AS V_Date, Max(VC.ManualCode) AS CommodityCode,  Sum(H.Qty) AS Qty, " &
                " Sum(H.Sales_Tax_Taxable_Amt) As Sales_Tax_Taxable_Amt, Sum(H.Sales_Tax) As Sales_Tax, " &
                " Sum(H.Additional_Sales_Tax) As Additional_Sales_Tax, Sum(H.Net_Amount) As Net_Amount, " &
                " Max(H.TINNo) As TINNo, Max(H.Unit) As Unit " &
                " FROM View_Vat H " &
                " LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type " &
                " LEFT JOIN Item I ON I.Code = H.Item  " &
                " LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description " &
                " LEFT JOIN VatCommodityCode VC ON VC.Code = I.VatCommodityCode  " &
                " " & mCondStr & " " &
                " Group By H.DocId " &
                " Order By V_Date "
    ...
    Private Sub FWriteForm24_Annexure_B(ByVal xlWorkBook As Excel.Workbook)
        ...
        mQry = " SELECT Max(H.RecId) AS InvoiceNo,  convert(NVARCHAR,Max(H.V_Date),103) AS V_Date, Max(VC.ManualCode) AS CommodityCode,  " &
                " Sum(H.Qty) AS Qty, " &
                " Sum(H.Sales_Tax_Taxable_Amt) As Sales_Tax_Taxable_Amt, Sum(H.Sales_Tax) As Sales_Tax, " &
                " Sum(H.Additional_Sales_Tax) As Additional_Sales_Tax, Sum(H.Net_Amount) As Net_Amount, " &
                " Max(H.TINNo) As TINNo, Max(H.Unit) As Unit " &
    ...
    Private Sub FWriteForm24_Annexure_C(ByVal xlWorkBook As Excel.Workbook)
        ...
        mQry = " SELECT Max(H.RecId) AS InvoiceNo,  convert(NVARCHAR,Max(H.V_Date),103) AS V_Date, Max(VC.ManualCode) AS CommodityCode,  Sum(H.Qty) AS Qty, " &
                " Sum(H.Sales_Tax_Taxable_Amt) As Sales_Tax_Taxable_Amt, Sum(H.Sales_Tax) As Sales_Tax, Sum(H.Additional_Sales_Tax) As Additional_Sales_Tax, " &
                " Sum(H.Net_Amount) As Net_Amount, Max(H.TINNo) As TINNo, " &
                " Max(H.Unit) As Unit, Max(SG.DispName) AS SellerName, IfNull(Max(SG.Add1),'') || ' ' || IfNull(Max(SG.Add2),'') || '-' || IfNull(Max(C.CityName),'') AS  SellerAddress, " &
                " Max(C.State) AS SellerState, Max(H.Form) As Form, Max(H.FormNo) As FormNo " &
                " FROM View_Vat H " &
                " LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type " &
